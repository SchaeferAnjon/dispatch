...
- **Only the Apple silicon package exists.** The README and the website mention an Intel package, but the releases currently hold only `macos-apple-silicon.zip`; an Intel machine has to build from source (`cd app && npm ci && npm run tauri build`, which needs Node 20.19+ / 22.12+, Rust, the Xcode command line tools and Python 3.12+). The in-app updater may offer the wrong package when it cannot find a matching architecture.
