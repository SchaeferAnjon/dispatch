...
- **项目回顾**：项目页的「回顾」页或任意目录里 `dispatch here`，一眼看到现状一段话（智谱 glm-5.3-flash）、最近 14 天时间线（任务进展与完成、git 提交、会话总结按天分组）、未完成任务及验收进度、本目录每个活会话在做什么和「可关/别关」。任务在 prime、会话列表、Herdr 标签里都以标题显示，task-id 只作后缀。
