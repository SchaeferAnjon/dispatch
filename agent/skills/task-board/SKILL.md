...
dispatch need-you "要用户做什么" -P 项目 -d "为什么、怎么做" [--task TASK_ID]   # 只有用户能做的事（发邮件、付款、登录、演示、决定）：记成「只能你做」，别只写在回复里
