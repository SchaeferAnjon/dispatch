...
dispatch quota                    # 各 Agent 的用量额度（Claude 5h/每周、Codex 5h/每周），快到顶时换个 Agent 干
dispatch rules show|status|sync   # 这台电脑所有 Agent 的共同规则（唯一来源 ~/.agents/rules/GLOBAL.md）；改完 sync 写入 Claude/Codex/ZCode 的全局文件
