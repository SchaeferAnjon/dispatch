...
export function SessionQuestion({ api, session, pending, onAnswered, onError }: { api: Api; session: SessionRef; pending: Pending; onAnswered: () => void; onError: (m: string) => void }) {
  ...
  return <section className="session-question" aria-label={t('Agent 的提问')}>
    <div className="sq-head"><span className="conversation-caption">{t('Agent 在问你')}</span><span className="muted small">{pending.questions.length > 1 ? t('{n} 题', { n: pending.questions.length }) : ''}</span></div>
    {pending.questions.map((q, qi) => <div key={qi} className="sq-q">
      <div className="sq-title">{q.header && <span className="sq-tag">{q.header}</span>}{q.question}{q.multiSelect && <span className="muted small"> · {t('可多选')}</span>}</div>
      <div className="sq-options">
        ...
        <label className={`sq-opt sq-other${answers[qi]?.other ? ' on' : ''}`}><span className="sq-n">{q.options.length + 1}</span><input placeholder={t('其他：自己写一句')} value={answers[qi]?.other ?? ''} disabled={busy} onChange={e => setAnswers(xs => xs.map((a, i) => i === qi ? { picks: [], other: e.target.value } : a))} /></label>
      </div>
    </div>)}
    <div className="sq-actions"><button className="btn primary" type="button" disabled={!ready || busy} onClick={() => void submit()}>{busy ? t('提交中…') : t('回答')}</button>{note && <span className="reply-error">{note}</span>}</div>
  </section>;
}
