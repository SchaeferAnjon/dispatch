...
import type { Activity, Host, Issue } from '../types';
...
export function ProjectHub({onDiscuss,focusSection,onSectionDone,archiveDays,flags,onFlag,connectionError,unavailable,rows,tasks,outcomes,api,me,selected,onProject,onOpen,onTask,onRead,onSummarize,onReload,onNew,loaded,hosts=[],owners={},onMoveProject}:Props){
  ...
  return <div className="project-hub"><button className="link" onClick={()=>{setQuery('');onProject(null);}}>‹ 全部项目</button><header className="hub-heading"><div><h2>{project.name}{starBtn(project.name)}{isArchived(flags,project.name)&&<span className="st sm open">已归档</span>}</h2><p>{project.sessions.filter(a=>!a.scheduled).length} 个会话 · {project.items.length} 个任务 · {project.results.length} 项成果</p></div><div className="hub-header-actions"><button className="btn" disabled={termBusy} onClick={()=>void openTerminal()} title={home?`在 ${shortPath(home)} 开一个不带 Agent 的 Herdr 终端标签`:'这个项目还没有记录到目录'}>{termBusy?'开终端…':'终端'}</button>{onDiscuss&&<button className="btn" onClick={()=>onDiscuss(project.name)} title="就这个项目的一个想法，让几个 Agent 各说一次并出结论">讨论…</button>}{(()=>{
    ...
    const from=homeAct?.host||'local';const ownerId=ownerHostId(owners[project.name],hosts);
    const where=hosts.find(h=>(h.local?'local':h.id)===(ownerId||from));
    const targets=home&&onMoveProject?hosts.filter(h=>(h.local?'local':h.id)!==from&&(h.local||h.online)):[];
    return <>{where&&<span className="host-chip project-owner" title={ownerId?'项目已交接给这台：新建会话默认开在这里':'项目目录目前在这台电脑上'}>{ownerId?'归 ':'在 '}{where.name}</span>}
