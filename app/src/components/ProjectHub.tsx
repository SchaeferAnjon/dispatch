...
import { UNGROUPED_PROJECT, conversationProject, sessionLifecycle, workspaceProjectOf } from '../activity';
...
export function ProjectFactsView({ name, doc, keys, draft, busy, err, onEdit, onChange, onCancel, onSave, onOpenPath, onCopy }: { name: string; doc: ProjectFactsDoc | null; keys: ProjectKey[] | null; draft: string | null; busy: boolean; err: string; onEdit: () => void; onChange: (t: string) => void; onCancel: () => void; onSave: () => void; onOpenPath: (p: string) => void; onCopy: (t: string) => void }) {
  ...
  return <section className="project-facts" aria-label={t('项目常用信息')}>
    <div className="hub-tools"><b>{t('常用信息')}</b><span className="chip">FACTS.md</span>{doc?.path && <code className="muted small doc-path" title={doc.path}>{shortPath(doc.path)}</code>}<span className="spacer" />
      {draft === null
        ? <>{doc?.exists && <button className="btn sm" onClick={() => onOpenPath(doc.path)}>{t('在 Finder 打开')}</button>}<button className="btn sm" disabled={busy || !doc} onClick={onEdit}>{doc?.exists ? t('编辑') : t('建一份')}</button></>
        : <><button className="btn sm" disabled={busy} onClick={onCancel}>{t('取消')}</button><button className="btn primary sm" disabled={busy} onClick={onSave}>{busy ? t('保存中…') : t('保存')}</button></>}</div>
    <p className="muted small">{t('只属于这个项目的短事实：服务器入口、登录方式、找谁。Agent 不会每次注入，需要时才')} <span className="mono">dispatch facts get -P {name} {t("主题")}</span> {t('读。')}</p>
    {err && <p className="err small" role="alert">{err}</p>}
    {draft !== null ? <textarea className="project-facts-editor" aria-label={t('编辑项目常用信息')} value={draft} onChange={(e) => onChange(e.target.value)} spellCheck={false} />
      : doc === null ? <p className="empty">{t('正在读…')}</p>
      : doc.content ? <div className="doc-reader"><Markdown src={doc.content} /></div>
      : <p className="empty">{doc.path ? t('还没有 FACTS.md。') : t('还不知道这个项目的目录（先在项目目录里开一次会话）。')}{hasLegacy && ` ${t('全局 FACTS.md 里有这个项目的旧节，见下。')}`}</p>}
    {hasLegacy && draft === null && <details className="project-facts-legacy"><summary className="muted small">{t('全局 FACTS.md 里的旧项目节 · {n}（可搬进项目 FACTS.md）', { n: doc!.legacy!.length })}</summary>{doc!.legacy!.map((l) => <div key={l.heading}><h4>{l.heading}</h4><Markdown src={l.body} /></div>)}</details>}
    <div className="hub-tools project-keys-head"><b>{t('密钥')}</b><span className="muted small">{keys ? t('{n} 个 · 只列名字和用途，值在「规则与资料 → 密钥与 API」', { n: keys.length }) : '…'}</span></div>
    {keys && keys.length > 0 && <div className="project-keys">{keys.map((k) => <div className="hub-task project-key" key={k.name}><code className="mono">{k.name}</code><span className="muted small">{k.note || t('（没写用途）')}</span><span className="spacer" /><button className="btn sm" title={`dispatch env get ${k.name}`} onClick={() => onCopy(`dispatch env get ${k.name}`)}>{t('复制取用命令')}</button></div>)}</div>}
    {keys && keys.length === 0 && <p className="empty small">{t('还没有登记到这个项目的密钥。在「规则与资料 → 密钥与 API」添加时项目填')} <span className="mono">{name}</span>{t('，或用命令')} <span className="mono">dispatch env set {t("名 值")} -P {name}</span></p>}
  </section>;
}
...
export function ProjectDocs({ api, name, docs, onReload }: { api: Api; name: string; docs: Doc[] | null; onReload: () => void }) {
  ...
  const open = async (doc: Doc) => {
    ...
    if (doc.html || doc.ext === 'html' || doc.ext === 'htm') {
      if (doc.host) { setErr(t('这份文档在 {host}，暂时只能在这里读 Markdown 文档', { host: doc.host_name || doc.host })); return; }
  ...
  const actions = (d: Doc) => <>{d.url ? <button className="btn sm" onClick={() => void openExternal(d.path)}>{t('打开链接')}</button>
    ...
    : <><button className="btn sm" onClick={() => void api.openPath(d.path).catch(() => {})}>{t('在 Finder 打开')}</button><button className="btn sm" onClick={() => void api.copy(d.path).catch(() => {})}>{t('复制路径')}</button></>}{d.source === 'registered' && <button className="btn sm" disabled={busy} onClick={() => void remove(d)}>{t('移除登记')}</button>}</>;
  if (opened) {
    const d = opened.doc;
    return <div className="doc-reader"><div className="hub-tools"><button className="btn sm" onClick={() => setOpened(null)}>{t('‹ 文档列表')}</button><span className="chip">{t(d.kind)}</span><b>{d.title}</b><code className="muted small doc-path" title={d.path}>{shortPath(d.path)}</code>{d.host && <span className="chip" title={d.path}>{d.host_name || d.host}</span>}<span className="spacer" />{actions(d)}</div><DocMedia api={api} project={name} id={d.id} dir={opened.dir} host={d.host}><Markdown src={opened.text} /></DocMedia></div>;
  }
...
// Keys are the Chinese source text; every use goes through t().
...
const reviewStatus = (s: string) => tr(s === 'closed' ? '已完成' : s === 'in_progress' ? '进行中' : s === 'blocked' ? '阻塞' : s === 'deferred' ? '搁置' : '待办');
...
function ReviewTimeline({ timeline, onOpen, onTask }: { timeline: ReviewDay[]; onOpen: (id: string) => void; onTask: (id: string) => void }) {
  ...
  const groups = useMemo(() => {
    ...
    if (loose.length) list.push({ task: '', title: t('未挂任务'), entries: loose });
    return list;
  }, [shown]);
  if (sorted.length === 0) return <p className="muted small">{t('这段时间没有记录。')}</p>;
  const row = (e: ReviewEntry, key: string, withDate: boolean) => {
    ...
    return <li className={`review-entry${go ? ' clickable' : ''}`} key={key}
      ...
      title={go ? (e.kind === 'session' ? t('打开这段会话') : t('打开这个任务')) : undefined}
      onClick={go || undefined}
      onKeyDown={go ? (ev) => { if (ev.key === 'Enter' || ev.key === ' ') { ev.preventDefault(); go(); } } : undefined}>
      <span className={`review-kind k-${e.kind}`}>{REVIEW_KIND[e.kind] ? t(REVIEW_KIND[e.kind]) : e.kind}</span>
      ...
      <span className="review-text clamp-2" title={e.text}>{e.text}</span>{e.host ? <span className="review-host muted small" title={t('来自 {host}', { host: e.host })}>{e.host}</span> : null}
    </li>;
  };
  return <>
    <div className="review-seg-row">
      <span className="review-seg-label">{t('最近')}</span>
      <div className="review-seg" role="group" aria-label={t('时间线显示范围')}>
        {[3, 7, 14].map((d) => <button key={d} type="button" className={days === d ? 'on' : ''} aria-pressed={days === d} onClick={() => setDays(d)}>{t('{n} 天', { n: d })}</button>)}
      </div>
      <span className="spacer" />
      <button type="button" className={`chip${expanded ? ' on' : ''}`} aria-pressed={expanded} onClick={() => setExpanded((v) => !v)} title={expanded ? t('收回到小框里') : t('去掉高度限制，整页滚动看')}>{expanded ? t('还原') : t('放大')}</button>
      <span className="review-seg-label">{t('分组')}</span>
      <div className="review-seg" role="group" aria-label={t('时间线分组方式')}>
        <button type="button" className={mode === 'task' ? 'on' : ''} aria-pressed={mode === 'task'} onClick={() => setMode('task')}>{t('按任务')}</button>
        <button type="button" className={mode === 'time' ? 'on' : ''} aria-pressed={mode === 'time'} onClick={() => setMode('time')}>{t('按时间')}</button>
      </div>
    </div>
    {mode === 'time'
      ? <div className={`review-timeline${expanded ? " expanded" : ""}`}>{shown.map((day) => {
        ...
        return <details className="review-day" key={day.day} open={open} onToggle={(e) => { const now = e.currentTarget.open; setOpenDays((prev) => (prev[day.day] === now ? prev : { ...prev, [day.day]: now })); }}>
          <summary className="review-day-head"><span className="mono">{day.day}</span><span className="muted small">{day.weekday}</span><span className="muted small review-day-count">· {t('{n} 条', { n: day.entries.length })}</span></summary>
          <ol className="review-entries">{entries.map((e, i) => row(e, `${e.ts}-${i}`, false))}</ol>
          {day.entries.length > 4 && (expanded
            ? <button type="button" className="review-more" onClick={() => setExpandedDays((p) => ({ ...p, [day.day]: false }))}>{t('收起')}</button>
            : <button type="button" className="review-more" onClick={() => setExpandedDays((p) => ({ ...p, [day.day]: true }))}>{t('展开剩余 {n} 条', { n: day.entries.length - 4 })}</button>)}
        </details>;
      })}</div>
      : <div className={`review-timeline${expanded ? " expanded" : ""}`}>{groups.map((g) => {
        ...
        return <details className="review-day" key={key} open={open} onToggle={(e) => { const now = e.currentTarget.open; setOpenGroups((prev) => (prev[key] === now ? prev : { ...prev, [key]: now })); }}>
          <summary className="review-day-head">
            {g.task
              ? <button type="button" className="link task-title review-group-title" title={t('打开这个任务')} onClick={(ev) => { ev.preventDefault(); ev.stopPropagation(); onTask(g.task); }}>{g.title || g.task}</button>
              : <span className="review-group-title">{g.title}</span>}
            <span className="muted small review-day-count">· {t('{n} 条', { n: g.entries.length })}</span>
          </summary>
          <ol className="review-entries">{entries.map((e, i) => row(e, `${e.ts}-${i}`, true))}</ol>
          {g.entries.length > 4 && (expanded
            ? <button type="button" className="review-more" onClick={() => setExpandedGroups((p) => ({ ...p, [key]: false }))}>{t('收起')}</button>
            : <button type="button" className="review-more" onClick={() => setExpandedGroups((p) => ({ ...p, [key]: true }))}>{t('展开剩余 {n} 条', { n: g.entries.length - 4 })}</button>)}
        </details>;
      })}</div>}
    {hidden > 0 && <button type="button" className="review-more review-hidden" onClick={() => setDays(14)}>{t('还有更早的 {n} 天 · 显示全部', { n: hidden })}</button>}
  </>;
}
...
export function ProjectReview({ api, data, busy, err, me, onOpen, onTask, onReload }: { api: Api; data: ReviewData | null; busy: boolean; err: string; me: string; onOpen: (id: string) => void; onTask: (id: string) => void; onReload: () => void }) {
  ...
  if (busy && !data) return <p className="empty">{t('正在读这个项目此刻的样子…')}</p>;
  ...
  const restore = async (s: ReviewSession) => {
    ...
    await openSession({ session_id: s.session_id, agent: s.agent, host: 'local', host_name: t('本机') });
  ...
  return <div className="review">
    <section className="review-block">
      <h3>{t('现状')}</h3>
      {summary.pending ? <p className="muted small">{t('正在读这个项目的会话和任务，写现状…')}</p>
        : summary.error ? <p className="muted small">{t('还没有项目总结：{error}', { error: summary.error })}</p>
        : summary.text ? <p className="review-summary">{summary.text}</p>
        : <p className="muted small">{t('还没有项目总结。')}</p>}
      {!data.detected && <p className="muted small">{t('任务板上没认出')} <span className="mono">{data.project}</span>{t('，按目录 {dir} 看。', { dir: shortPath(data.cwd) })}</p>}
    </section>
    <section className="review-block">
      <h3>{t('最近 {n} 天', { n: data.timeline_days })}</h3>
      <ReviewTimeline key={data.project} timeline={data.timeline} onOpen={onOpen} onTask={onTask} />
    </section>
    <section className="review-block">
      <h3>{t('还没做完')} <span className="review-count">{data.open_tasks.length}</span></h3>
      {data.open_tasks.length === 0 ? <p className="muted small">{t('没有未完成任务。')}</p>
        : <div className="review-tasks">{data.open_tasks.map((task) => <div className="review-task" key={task.id}>
          <div className="review-task-head">
            <span className={`review-mark${task.status === 'in_progress' ? ' prog' : ''}`} aria-hidden>{task.status === 'in_progress' ? '◐' : '○'}</span>
            <button className="link task-title" onClick={() => onTask(task.id)}>{task.title}</button>
            <code className="muted small review-ref">{task.id}</code>
          </div>
          <div className="review-task-meta">
            {task.acceptance_total > 0 && <span className="chip">{t('{done}/{total} 验收', { done: task.acceptance_done, total: task.acceptance_total })}</span>}
            {task.assignee && <span className="muted small">{actorOf(task.assignee, me)?.name || task.assignee}</span>}
            {task.last_at ? <span className="muted small">{ago(task.last_at)}</span> : null}
          </div>
          {task.last_note && <p className="review-note muted small">{task.last_note}</p>}
        </div>)}</div>}
    </section>
    <section className="review-block">
      <h3>{t('本目录活会话')} <span className="review-count">{data.sessions.length}</span></h3>
      {data.sessions.length === 0 ? <p className="muted small">{t('这个目录下没有正在运行的会话。')}</p>
        : <div className="review-sessions">{data.sessions.map((s) => <div className="review-session" key={`${s.agent}:${s.session_id}`}>
          <div className="review-session-head">
            ...
            {closed[s.session_id]
              ? <span className="review-verdict closed">{t('已关闭')}</span>
              : <button type="button" className={`review-verdict${s.verdict === '别关' ? ' hold' : ' ok'}${confirming === s.session_id ? ' confirm' : ''}`} disabled={closing === s.session_id} onClick={() => (s.verdict === '可关' ? (confirming === s.session_id ? void closeSession(s) : setConfirming(s.session_id)) : onOpen(s.session_id))} title={s.verdict === '可关' ? t('关闭这个会话：关掉它的 Herdr 标签/进程，之后可点「恢复」续上') : t('不安全的关闭，点开这段会话看看')}>{closing === s.session_id ? t('关闭中…') : s.verdict === '可关' && confirming === s.session_id ? t('确认关闭？') : t(s.verdict)}</button>}
            <button type="button" className="review-restore" onClick={() => void restore(s)} title={t('在 Herdr 里恢复/打开这段会话')}>{t('恢复')}</button>
          </div>
          <div className="review-task-meta">
            <span className="chip">{s.agent}</span>
            <span className="muted small">{REVIEW_STATE[s.state] ? t(REVIEW_STATE[s.state]) : s.state}</span>
            {s.last_at ? <span className="muted small">{ago(s.last_at)}</span> : null}
            {s.files_count > 0 && <span className="muted small">{t('动过 {n} 个文件', { n: s.files_count })}</span>}
          </div>
          ...
          {s.tasks.length > 0 && <div className="review-links">{s.tasks.map((task) => <button key={task.id} className="chip" onClick={() => onTask(task.id)}>{task.title} · {reviewStatus(task.status)}</button>)}</div>}
        </div>)}</div>}
    </section>
  </div>;
}
...
export function TaskRelations({issue,rows,api,onSaved,onOpen}:{issue:Issue;rows:Activity[];api:Api;onSaved:()=>void;onOpen:(id:string)=>void}) {
  ...
  return <div className="task-relations"><div className="task-links">{ids.length?ids.map(id=><button key={id} className="chip" onClick={()=>onOpen(id)}>{originSession(issue)===id?t('发起'):t('参与')} · {rows.find(a=>a.session_id===id)?.title||id.slice(0,8)} ↗</button>):<span className="muted small">{t('没记录（Agent 建任务时通常会自动记）')}</span>}<button className="link" onClick={()=>{setOrigin(originSession(issue));setParticipants(ids);setEdit(!edit);}}>{t('指定是哪次会话')}</button></div>{edit&&<div className="relation-editor"><label>{t('发起会话')}<select aria-label={t('发起会话')} value={origin} onChange={e=>setOrigin(e.target.value)}><option value="">{t('暂不指定')}</option>{options.map(a=><option key={a.session_id} value={a.session_id}>{conversationProject(a)} · {a.title}</option>)}{missing.map(id=><option key={id} value={id}>{t('{id}（历史会话）',{id})}</option>)}</select></label><details><summary>{t('参与会话 · 可选多个')}</summary><div className="relation-options">{options.map(a=><label key={a.session_id}><input type="checkbox" checked={participants.includes(a.session_id)} onChange={e=>setParticipants(old=>e.target.checked?[...old,a.session_id]:old.filter(x=>x!==a.session_id))}/>{a.title}</label>)}</div></details><button className="btn primary sm" disabled={busy} onClick={()=>void save()}>{t('保存')}</button><button className="btn sm" disabled={busy} onClick={()=>setEdit(false)}>{t('取消')}</button>{error&&<p role="alert">{error}</p>}</div>}</div>;
}
...
function OutcomeEditor({project,rows,tasks,initial,api,onSaved,onCancel}:{project:string;rows:Activity[];tasks:Issue[];initial?:Issue;api:Api;onSaved:()=>void;onCancel:()=>void}){
  ...
  return <form className="outcome-editor" onSubmit={e=>{e.preventDefault();void save();}}><h3>{initial?t('编辑成果'):t('登记成果')}</h3><label>{t('成果名称')}<input aria-label={t('成果名称')} required maxLength={200} value={title} onChange={e=>setTitle(e.target.value)} placeholder={t('例如：Dispatch 新版 · 项目工作台')}/></label><label>{t('交付内容与查看入口')}<textarea aria-label={t('交付内容与查看入口')} required rows={5} value={body} onChange={e=>setBody(e.target.value)} placeholder={t('写明交付了什么，附上文档、截图、版本或代码链接（支持 Markdown）')}/></label><div className="outcome-sources"><details open><summary>{t('来源任务 · 可选多个')}</summary><div className="relation-options">{tasks.map(i=><label key={i.id}><input type="checkbox" checked={taskIds.includes(i.id)} onChange={e=>setTaskIds(old=>e.target.checked?[...old,i.id]:old.filter(x=>x!==i.id))}/>{i.title}</label>)}</div></details><details><summary>{t('补充来源会话 · 已选任务的会话会一并关联')}</summary><div className="relation-options">{rows.map(a=><label key={a.key}><input type="checkbox" checked={sessionIds.includes(a.session_id)} onChange={e=>setSessionIds(old=>e.target.checked?[...old,a.session_id]:old.filter(x=>x!==a.session_id))}/>{a.title}</label>)}</div></details></div>{error&&<p role="alert">{t('保存失败：{error}',{error})}</p>}<button className="btn primary" disabled={busy||!title.trim()||!body.trim()}>{t('保存成果')}</button><button className="btn" type="button" disabled={busy} onClick={onCancel}>{t('取消')}</button></form>;
}

type Props={onDiscuss?:(name:string)=>void;focusSection?:{section:"review"|"sessions";token:number}|null;onSectionDone?:()=>void;archiveDays:number;flags:ProjectFlags;onFlag:(name:string,change:{starred?:boolean;archived?:boolean;alias?:string})=>void;connectionError:boolean;unavailable:string[];rows:Activity[];tasks:Issue[];outcomes:Issue[];api:Api;me:string;selected:string|null;onProject:(name:string|null)=>void;onOpen:(id:string)=>void;onTask:(id:string)=>void;onRead:(a:Activity)=>Promise<void>;onSummarize?:(a:Activity)=>Promise<void>;onReload:()=>void;onNew:(a?:Activity)=>void;onCreated?:(name:string,dir:string,startSession:boolean)=>void;workspaceRoots?:string[];loaded:boolean;hosts?:Host[];owners?:Record<string,ProjectOwner>;moveJobs?:MoveJob[];onMoveProject?:(project:string,cwd:string,fromHost:string,to:Host)=>Promise<void>};
...
function ProjectWiki({entries,project,onTask}:{entries:WikiEntry[]|null;project:string;onTask:(id:string)=>void}){
  ...
  if(entries===null)return <p className="empty">{t('正在读这个项目的知识库…')}</p>;
  ...
  return <div className="project-wiki">
    <div className="hub-tools"><div className="views">{[['all','全部',entries.length],...Object.entries(WIKI_KIND).filter(([k])=>counts[k]).map(([k,l])=>[k,l,counts[k]])].map(([k,l,n])=><button key={String(k)} className={kind===k?'on':''} onClick={()=>setKind(String(k))}>{t(String(l))} {n}</button>)}</div><label className="search"><input placeholder={t('搜这个项目的知识库…')} value={q} onChange={e=>setQ(e.target.value)}/></label></div>
    {!entries.length&&<p className="empty">{t('这个项目还没有知识库条目。Agent 踩坑或做对后用')} <code>dispatch wiki add --kind pit/win -P {project}</code> {t('记下来，就会出现在这里。')}</p>}
    {!!entries.length&&!shown.length&&<p className="empty">{t('没有匹配的条目。')}</p>}
    <div className="wiki-list">{shown.map(e=><div key={e.key} className={`wiki-item k-${e.kind}`}><div className="wiki-head"><span className={`review-kind k-${e.kind==='pit'?'task':e.kind==='win'?'done':'session'}`}>{WIKI_KIND[e.kind]?t(WIKI_KIND[e.kind]):e.kind}</span>{e.task&&<button className="link mono" onClick={()=>onTask(e.task!)}>{e.task}</button>}<span className="muted small mono">{e.key}</span></div><p className="wiki-text">{e.text}</p>{Object.entries(e.fields||{}).filter(([,v])=>v).map(([k,v])=><p key={k} className="wiki-field"><b>{t(k.replace(/[【】]/g, ""))}</b>{v}</p>)}</div>)}</div>
  </div>;
}

export function ProjectHub({moveJobs=[],onDiscuss,onCreated,workspaceRoots,focusSection,onSectionDone,archiveDays,flags,onFlag,connectionError,unavailable,rows,tasks,outcomes,api,me,selected,onProject,onOpen,onTask,onRead,onSummarize,onReload,onNew,loaded,hosts=[],owners={},onMoveProject}:Props){
  const t=useT();
  ...
  const groups=useMemo(()=>projectGroups(rows,tasks,outcomes,flags),[rows,tasks,outcomes,flags]);
  const [creating,setCreating]=useState(false);
  // A real project: it has tasks or outcomes, was linked by hand, was created in the app with a folder, is starred,
  // or its conversations live under a workspace root (~/Projects/<name>: the person put it there on purpose).
  const primary=(p:typeof groups[number])=>p.items.length>0||p.results.length>0||p.sessions.some(a=>!!a.project_override)||!!flags[p.name]?.dir||isStarred(flags,p.name)||p.sessions.some(a=>!!workspaceProjectOf(a.cwd,workspaceRoots));
  const ranked=useMemo(()=>rankProjects(groups,flags),[groups,flags]);
  const project=groups.find(p=>p.name===selected);
  const starBtn=(name:string)=><button className={`star${isStarred(flags,name)?' on':''}`} onClick={e=>{e.stopPropagation();onFlag(name,{starred:!isStarred(flags,name)});}} title={isStarred(flags,name)?t('取消收藏'):t('收藏：置顶，近期重点关注')} aria-label={isStarred(flags,name)?t('取消收藏 {name}',{name}):t('收藏 {name}',{name})}>{isStarred(flags,name)?'★':'☆'}</button>;
  ...
  const ownerOf=(p:typeof groups[number]):{name:string;why:string}=>{
    ...
    if(name)return {name,why:owners[p.name]?t('项目已交接给这台：新建会话默认开在这里'):t('项目目录目前在这台电脑上')};
    ...
    return fromTasks?{name:fromTasks,why:t('按这个项目的任务在哪台电脑上做过推断')}:{name:'',why:t('还没有归属、目录或任务记录')};
  };
  const ownerBadge=(p:typeof groups[number])=>{
    const {name,why}=ownerOf(p);
    return <span className={`project-owner${name?'':' unknown'}`} title={why}>{name||t('未指定')}</span>;
  };
  ...
  const moveBar=(name:string,compact=false)=>{const j=moveOf(name);if(!j)return null;return <div className={`move-progress${compact?' compact':''}`} role="progressbar" aria-valuenow={j.percent} aria-valuemin={0} aria-valuemax={100} title={t('正在迁移到 {host}：{stage}',{host:j.to,stage:j.label})}><div className="move-bar"><i style={{width:`${Math.max(2,j.percent)}%`}}/></div><span className="move-text">{t('迁移到 {host}',{host:j.to})} · {j.label} · {j.percent}%</span></div>;};
  ...
  if(!project)return <div className="project-hub"><header className="hub-heading"><div><h2>{t('从项目继续工作')}</h2><p>{t('项目 → 会话 → 任务 → 成果')}</p><span className="muted small">{connectionError?t("更新中断，正在重新连接"):loaded?t("每 3 秒同步会话活动"):t("正在连接…")}</span></div><div className="hub-header-actions">{onCreated&&<button className="btn" onClick={()=>setCreating(true)} title={t('先建项目再开会话：选文件夹或新建文件夹')}>{t('新建项目')}</button>}<button className="btn primary" onClick={()=>onNew()}>{t('新建会话')}</button></div></header>{creating&&onCreated&&<NewProject api={api} onClose={()=>setCreating(false)} onCreated={(n,d,start)=>{setCreating(false);onCreated(n,d,start);}}/>}{unavailable.length>0&&<p className="connection-note">{t('{hosts} 暂时无法连接，保留已读取记录。',{hosts:unavailable.join(t("、"))})}</p>}<div className="hub-list-tools"><label className="search"><input placeholder={t('搜索项目…')} value={query} onChange={e=>setQuery(e.target.value)}/></label><label className="hub-sort">{t('排序')}<select aria-label={t('项目排序')} value={sortBy} onChange={e=>setSortBy(e.target.value as typeof sortBy)}><option value="recent">{t('最近活动')}</option><option value="name">{t('名称')}</option><option value="sessions">{t('会话数')}</option><option value="tasks">{t('待完成任务')}</option><option value="owner">{t('所属电脑')}</option></select></label></div>{!loaded&&<p className="empty">{t('正在读取项目与会话…')}</p>}{(()=>{const openTasks=(p:typeof groups[number])=>p.items.filter(i=>i.status!=='closed').length;const sorters:Record<typeof sortBy,(a:typeof groups[number],b:typeof groups[number])=>number>={recent:()=>0,name:(a,b)=>a.name.localeCompare(b.name,'zh-Hans-CN'),sessions:(a,b)=>b.sessions.length-a.sessions.length||b.last-a.last,tasks:(a,b)=>openTasks(b)-openTasks(a)||b.last-a.last,owner:(a,b)=>(ownerOf(a).name||'～').localeCompare(ownerOf(b).name||'～','zh-Hans-CN')||b.last-a.last};const list=(showArchived?ranked.archived:ranked.active).filter(p=>(showArchived||showOther||query.trim()||primary(p))&&p.name.toLowerCase().includes(query.toLowerCase())).sort(sorters[sortBy]);const WEEK=7*86400,now=Date.now()/1000;const info=(p:typeof list[number])=>{const active=p.sessions.filter(a=>!a.scheduled&&sessionLifecycle(a,archiveDays)!=='archived');const unread=active.filter(a=>a.unread).length;const doing=p.items.find(i=>i.status==='in_progress');const openCount=p.items.filter(i=>i.status!=='closed').length;const live=unread>0||!!doing||active.some(a=>a.state==='working'&&!a.stale)||(p.last>0&&now-p.last<WEEK);
    // One line that says what the project is up to, never a raw chat reply.
    const blurb=doing?t('进行中 · {title}',{title:doing.title}):p.results[0]?t('最新成果 · {title}',{title:p.results[0].title}):active[0]?t('最近会话 · {title}',{title:active[0].title}):p.items.length?t('{n} 个任务，还没有会话',{n:p.items.length}):t('还没有会话或任务');return{active,unread,openCount,live,blurb};};const cards=list.filter(p=>info(p).live),quiet=list.filter(p=>!info(p).live);return <><div className="project-cards">{cards.map(p=>{const x=info(p);return <div className="project-card" data-project={p.name} role="button" tabIndex={0} key={p.name} onClick={()=>open(p.name)} onKeyDown={e=>{if(e.target===e.currentTarget&&e.key==='Enter')open(p.name);}} title={t('右键：收藏、归档、新建会话')}><div><h3>{projectLabel(flags,p.name)}</h3>{starBtn(p.name)}{ownerBadge(p)}{x.unread>0&&<span className="activity-badge new">{t('{n} 个会话未读',{n:x.unread})}</span>}</div>{moveBar(p.name,true)}<p>{x.blurb}</p><div className="muted small">{t('{n} 个会话',{n:x.active.length})} · {t('{n} 个待完成任务',{n:x.openCount})} · {t('{n} 项成果',{n:p.results.length})}</div><div className="project-card-foot"><span className="link">{primary(p)?t("进入项目 →"):t("查看目录会话 →")}</span><span className="muted small">{p.last?`${ago(p.last)}`:''}</span></div></div>;})}</div>{quiet.length>0&&<div className="project-quiet"><h4 className="muted small">{t('最近没有动静 · {n}',{n:quiet.length})}</h4>{quiet.map(p=>{const x=info(p);return <div className="home-rest-item opens" data-project={p.name} role="button" tabIndex={0} key={p.name} onClick={()=>open(p.name)} onKeyDown={e=>{if(e.key==='Enter')open(p.name);}} title={t('右键：收藏、归档、新建会话')}><span className="proj" style={{background:projectColor(p.name)}}/><b>{projectLabel(flags,p.name)}</b>{starBtn(p.name)}{ownerBadge(p)}<span className="muted small">{t('{n} 个会话',{n:x.active.length})} · {t('{n} 项未完成',{n:x.openCount})}{p.results.length?` · ${t('{n} 项成果',{n:p.results.length})}`:''}</span><span className="muted small right">{p.last?`${ago(p.last)}`:''}</span></div>;})}</div>}</>;})()}{showArchived&&ranked.archived.length===0&&<p className="empty">{t('没有归档的项目。')}</p>}<div className="hub-list-foot"><button className="btn" onClick={()=>{setShowOther(!showOther);setShowArchived(false);}}>{showOther&&!showArchived?t("收起其他目录"):t('其他目录与未归类 · {n}',{n:ranked.active.filter(p=>!primary(p)).length})}</button><button className={`btn${showArchived?' on':''}`} onClick={()=>setShowArchived(!showArchived)}>{showArchived?t("返回项目列表"):t('已归档 · {n}',{n:ranked.archived.length})}</button></div></div>;
  ...
  const openTerminal=async(kind?:string)=>{
    ...
    try{
      ...
      const out=await api.on(terminalHost,[...args,'--cwd',terminalCwd,'--label',project.name,'--json']);
      const d=JSON.parse(out.slice(Math.max(0,out.indexOf('{'))));
      if(d.error)throw new Error(String(d.error));
      const agentName=KINDS.find(([k])=>k===kind)?.[1]||kind;
      const what=kind?t('{agent} 会话',{agent:String(agentName)}):t('终端');
      setTerminalNotice({project:project.name,text:d.app?t('已在 {host} 的 {app} 打开{what}：{cwd}',{host:d.host||terminalMachine,app:d.app,what,cwd:d.cwd}):`${t('已在 {host} 创建{what}，但未能显示窗口。',{host:d.host||terminalMachine,what:kind?what:t('终端标签')})}${d.hint||t('请检查那台电脑的终端窗口。')}`,error:!d.app});
  ...
  return <div className="project-hub"><button className="link" onClick={()=>{setQuery('');onProject(null);}}>{t('‹ 全部项目')}</button><header className="hub-heading"><div><h2 onDoubleClick={()=>setRenaming(projectLabel(flags,project.name))} title={t('双击改显示名（标签和目录不变）')}>{renaming!==null?<input className="project-rename" autoFocus value={renaming} maxLength={80} aria-label={t('项目显示名')} onChange={e=>setRenaming(e.target.value)} onBlur={()=>commitRename(project.name)} onKeyDown={e=>{if(e.key==='Enter'){e.preventDefault();commitRename(project.name);}if(e.key==='Escape'){e.preventDefault();setRenaming(null);}}}/>:projectLabel(flags,project.name)}{flags[project.name]?.alias&&renaming===null&&<span className="muted small project-raw" title={t('原名：标签和目录用的')}>{project.name}</span>}{starBtn(project.name)}{ownerBadge(project)}{isArchived(flags,project.name)&&<span className="st sm open">{t('已归档')}</span>}</h2>{terminalCwd&&<p className="mono muted small project-dir" title={terminalCwd}>{terminalCwd}</p>}{moveBar(project.name)}<p>{t('{n} 个会话',{n:project.sessions.filter(a=>!a.scheduled).length})} · {t('{n} 个任务',{n:project.items.length})} · {t('{n} 项成果',{n:project.results.length})}</p></div><div className="hub-header-actions"><button className="btn" disabled={termBusy||!terminalCwd} aria-haspopup="menu" aria-expanded={!!terminalMenu} onClick={e=>{const r=e.currentTarget.getBoundingClientRect();setTerminalMenu({x:r.left,y:r.bottom+6,from:e.currentTarget});}} title={terminalCwd?t('选择 Agent，在 {host} 的 {dir} 打开终端会话',{host:terminalMachine,dir:shortPath(terminalCwd)}):t('这个项目还没有记录到目录')}>{termBusy?t('正在打开…'):`${t('终端 · {host}',{host:terminalMachine})} ▾`}</button>{terminalMenu&&<MenuPanel x={terminalMenu.x} y={terminalMenu.y} returnTo={terminalMenu.from} label={t('选择终端 Agent')} title={t('在 {host} 打开',{host:terminalMachine})} onClose={()=>setTerminalMenu(null)}>{KINDS.map(([kind,label])=><MenuItem key={kind} onClick={()=>void openTerminal(kind)}>{label}</MenuItem>)}<hr/><MenuItem onClick={()=>void openTerminal()}>{t('普通终端')}</MenuItem></MenuPanel>}{onDiscuss&&<button className="btn" onClick={()=>onDiscuss(project.name)} title={t('就这个项目的一个想法，让几个 Agent 各说一次并出结论')}>{t('讨论…')}</button>}{(()=>{
    ...
    return <>
      {targets.map(h=>{const j=moveOf(project.name);return <button key={h.id} className="btn" disabled={!!moving||!!j} onClick={async()=>{setMoving(h.id);try{await onMoveProject!(project.name,cwd!,from,h);}finally{setMoving(null);}}} title={j?t('正在迁移到 {host}：{stage}',{host:j.to,stage:j.label}):t('把项目目录、Git 和这边在跑的会话交给 {host}：先预检，确认后才动手',{host:h.name})}>{j&&j.to===h.name?t('迁移中 {n}%',{n:j.percent}):moving===h.id?t('预检中…'):t('迁移到 {host}',{host:h.name})}</button>;})}</>;
  })()}<button className="btn primary" onClick={()=>onNew(projectHome(project.sessions))}>{t('在此项目新建会话')}</button><button className="btn" onClick={()=>onFlag(project.name,{archived:!isArchived(flags,project.name)})} title={isArchived(flags,project.name)?t('恢复到工作台和项目列表'):t('做完了、暂时不用：从工作台和项目列表隐藏，随时可找回')}>{isArchived(flags,project.name)?t('取消归档'):t('归档')}</button></div></header><div className="hub-tabs views">{[['review',t('项目回顾')],['sessions',t('会话'),project.sessions.filter(a=>!a.scheduled).length],['tasks',t('任务'),project.items.length],['outcomes',t('成果'),project.results.length],['unassigned',t('待归属任务'),unassigned.length],['folders',t('目录'),dirs.length],['docs',t('文档'),docs?docs.length:'…'],['wiki',t('知识库'),wiki?wiki.length:'…']].map(([id,label,n])=><button className={tab===id?'on':''} key={id} onClick={()=>{setTab(String(id));setEditor(false);}}>{label}{n!=null?` ${n}`:''}</button>)}</div>
  ...
  {tab==='sessions'&&<><div className="hub-tools"><input aria-label={t('搜索项目会话')} placeholder={t('搜索这个项目的会话…')} value={query} onChange={e=>setQuery(e.target.value)}/><button className={`btn sm${archived?' on':''}`} onClick={()=>{setArchived(!archived);setScheduled(false);}} title={t('手动归档，或超过 {n} 天没有活动',{n:archiveDays})}>{archived?t('返回最近会话'):t('已归档 {n}',{n:archivedCount})}</button><button className="btn sm" onClick={()=>{setScheduled(!scheduled);setArchived(false);}}>{scheduled?t('返回普通会话'):t('定时会话 {n}',{n:project.sessions.filter(a=>a.scheduled).length})}</button></div><ConversationRows rows={visible} me={me} onOpen={onOpen} onRead={onRead} onSummarize={onSummarize} taskContent={a=>{const linked=project.items.filter(i=>linkedSessions(i).includes(a.session_id));const rev=verdicts.get(a.session_id);return <div className="conversation-tasks-line">{rev?<><span className={`review-verdict${rev.verdict==='别关'?' hold':''}`}>{t(rev.verdict)}</span>{rev.reason&&<span className="muted small">{rev.reason}</span>}</>:null}{linked.length?linked.map(i=><button key={i.id} className="chip" onClick={()=>onTask(i.id)}>{i.title}<span className="muted small"> · {i.status==='closed'?t('已完成'):i.status==='in_progress'?t('进行中'):t('待办')}</span></button>):<span className="muted small">{t('没有明确关联的任务')}</span>}</div>;}}/>{visible.length===0&&<p className="empty">{archived?t('没有归档的会话。'):t('这个分类没有会话。')}</p>}</>}
  {tab==='tasks'&&(()=>{const needs=project.items.filter(i=>isNeedsYou(i)&&i.status!=='closed');const rest=project.items.filter(i=>!(isNeedsYou(i)&&i.status!=='closed'));const closedAt=(i:Issue)=>Date.parse(i.closed_at||i.updated_at||'')/1000||0;const week=Date.now()/1000-7*86400;const doneAll=rest.filter(i=>columnOf(i)==='done').sort((a,b)=>closedAt(b)-closedAt(a));const doneRecent=doneAll.filter(i=>closedAt(i)>=week);
    // The label doubles as the column key; t() only at display.
    ...
    if(doneView){const q=doneQuery.trim().toLowerCase();const hits=doneAll.filter(i=>!q||i.title.toLowerCase().includes(q)||(i.description||'').toLowerCase().includes(q)||(i.close_reason||'').toLowerCase().includes(q));
      ...
      const label=(k:number)=>{const end=new Date(Date.now()-k*7*86400*1000),start=new Date(end.getTime()-6*86400*1000);const f=(d:Date)=>t('{m}月{d}日',{m:d.getMonth()+1,d:d.getDate()});return k===0?t('最近 7 天（{from}–{to}）',{from:f(start),to:f(end)}):`${f(start)}–${f(end)}`;};
      return <div className="done-archive"><div className="hub-tools"><button className="link" onClick={()=>setDoneView(false)}>{t('‹ 返回任务板')}</button><input aria-label={t('搜索已完成任务')} placeholder={t('搜标题、描述、完成说明…')} value={doneQuery} onChange={e=>setDoneQuery(e.target.value)}/><span className="muted small">{t('{hits} / {total} 项已完成',{hits:hits.length,total:doneAll.length})}</span></div>{[...groups.keys()].sort((a,b)=>a-b).map(k=><section key={k} className="done-week"><h4>{label(k)}<span className="n">{groups.get(k)!.length}</span></h4>{groups.get(k)!.map(taskRow)}</section>)}{!hits.length&&<p className="empty">{t('没有匹配的已完成任务。')}</p>}</div>;}
    return <><p className="muted">{t('任务显示发起和参与会话；历史任务没有明确关系时保留待归属，不根据提及次数猜测。')}</p>{err&&<p className="danger">{err}</p>}<div className="hub-board">{cols.map(([label,list])=><section className={`hub-col${label==='只能你做'?' needs-you':''}`} key={label}><h4>{t(label)}<span className="n">{list.length}</span>{label==='已完成'&&doneAll.length>doneRecent.length&&<button className="link small" onClick={()=>setDoneView(true)}>{t('全部 {n} ›',{n:doneAll.length})}</button>}</h4>{label==='只能你做'&&!list.length&&<p className="muted small">{t('Agent 遇到只有你能做的事（发邮件、付款、登录、演示）会记在这里，做完打勾。')}</p>}{label==='已完成'&&!list.length&&doneAll.length>0&&<p className="muted small">{t('最近 7 天没有完成的任务；更早的在「全部」里。')}</p>}{list.map(i=>label==='只能你做'?<div className="hub-task needs-you-task" key={i.id}><label className="tick" title={t('做完了，打勾关掉')}><input type="checkbox" onChange={()=>void tick(i)} aria-label={t('做完了：{title}',{title:i.title})}/></label><div className="needs-you-body"><button className="link task-title" onClick={()=>onTask(i.id)}>{i.title}</button>{i.description&&<p className="muted small clamp-2">{i.description}</p>}</div></div>:taskRow(i))}</section>)}</div></>;})()}
  {tab==='unassigned'&&<><p className="muted">{t('任务显示发起和参与会话；历史任务没有明确关系时保留待归属，不根据提及次数猜测。')}</p>{unassigned.map(taskRow)}{!unassigned.length&&<p className="empty">{t('任务都已有明确关联。')}</p>}</>}
  {tab==='folders'&&<><p className="muted">{t('这个项目的会话在哪些文件夹里发生过。')}</p>{dirs.map(([d,info])=><div className="hub-task hub-dir" key={d}><div className="hub-dir-head"><code title={d}>{short(d)}</code><span className="muted small">{t('{n} 个会话',{n:info.count})} · {t('最近 {ago}',{ago:ago(info.last)})}</span></div><div className="task-links"><button className="btn sm" onClick={()=>onNew(info.latest)}>{t('在此目录新建会话')}</button><button className="btn sm" onClick={()=>api.openPath(d).catch(()=>{})}>{t('在 Finder 打开')}</button><button className="btn sm" onClick={()=>api.copy(`cd '${d}'`).catch(()=>{})}>{t('复制 cd')}</button></div></div>)}{!dirs.length&&<p className="empty">{t('没有记录到工作目录。')}</p>}</>}
  ...
  {tab==='outcomes'&&<>{editor!==false?<OutcomeEditor key={editor?.id||"new"} project={project.name} rows={project.sessions} tasks={project.items} initial={editor||undefined} api={api} onCancel={()=>setEditor(false)} onSaved={()=>{setEditor(false);onReload();}}/>:<button className="btn primary" onClick={()=>setEditor(null)}>{t('登记成果')}</button>}{project.results.map(r=><article className="outcome-card" key={r.id}><div className="hub-heading"><h3>{r.title}</h3><button className="btn sm" onClick={()=>setEditor(r)}>{t('编辑')}</button></div><MediaProvider api={api} session={rows.find(a=>linkedSessions(r).includes(a.session_id))}><Markdown src={r.description||''}/></MediaProvider><div className="task-links">{sourceTasks(r).map(id=><button key={id} className="chip" onClick={()=>onTask(id)}>{t('任务')} · {tasks.find(i=>i.id===id)?.title||id}</button>)}{linkedSessions(r).map(id=><button key={id} className="chip" onClick={()=>onOpen(id)}>{t('会话')} · {rows.find(a=>a.session_id===id)?.title||id.slice(0,8)} ↗</button>)}</div></article>)}{!project.results.length&&editor===false&&<p className="empty">{t('还没有登记成果。可以把多个任务、多个会话的交付汇总在这里。')}</p>}<details className="hub-history"><summary>{t('历史完成记录 · {n} 项',{n:project.items.filter(i=>i.status==='closed').length})}</summary><p className="muted small">{t('这些是任务完成说明，尚未整理为独立成果。')}</p>{project.items.filter(i=>i.status==='closed').map(i=><div className="hub-task" key={i.id}><button className="link" onClick={()=>onTask(i.id)}>{i.title}</button><p>{i.close_reason||t('打开任务查看完成说明')}</p></div>)}</details></>}
  </div>;
}
