...
import { t as tr, useT, useLocalePref, setLocale, LOCALES, LOCALE_NAMES, type LocalePref } from "../i18n";
...
export function summaryOptionLabel(p: SummaryProvider): string { return `${p.label} · ${p.subscription ? tr("订阅") : p.configured ? tr("已配 Key") : tr("缺 Key")}`; }
...
export function SettingsView({ settings, onSave, theme, onTheme, api, summaryProviders = [], onPhone, phoneQr, phoneHost, onPhoneHost, onScreen, screenReady, screen, onScreenSetup, hosts = [], onSetup, onRenameHost, onDeleteHost, onRedetectHost, onTestNotify, update, onCheckUpdate, onApplyUpdate }: Props) {
  ...
  const saveKey = async () => {
    ...
    try {
      ...
      if (r.error || !r.ok) { setSummaryError(r.error || t("保存 Key 失败")); return; }
      setKeyValue("");
      await loadSummary();
    ...
    <div className="settings">
      <section className="settings-card">
        ...
        <label className="settings-row">
          <div><b>{t("界面语言")}</b><p>{t("跟随系统时按 macOS / 浏览器的语言自动选择；选定后立即生效。")}</p></div>
          <select value={localePref} onChange={(e) => setLocale(e.target.value as LocalePref)} aria-label={t("界面语言")}>
      ...
      <section className="settings-card">
        <h3>{t("会话")}</h3>
        <label className="settings-row">
          <div><b>{t("普通会话多少天没有活动后自动归档")}</b><p>{t("收藏（追踪中）的会话不受影响；归档的会话在会话页「已归档」和 ⌘K 里还能找到。填 0 表示永不自动归档。")}</p></div>
          <span className="settings-num"><input type="number" min={0} max={3650} value={draft.session_archive_days} onChange={(e) => num("session_archive_days", e.target.value, 3650)} /> {t("天")}</span>
        </label>
        <label className="settings-row summary-model-row">
          <div><b>{t("总结用的模型")}</b><p>{t("订阅模型不用 Key、计入用量；API Key 的来自「环境」页里配的密钥。空着就自动选：设置 → 环境变量 SUMMARY_MODEL → 第一个可用的。下面决定总结显示在哪些地方。")}</p></div>
          <select value={draft.summary_model} onChange={(e) => { setDraft({ ...draft, summary_model: e.target.value }); setKeyValue(""); setSummaryError(""); }} aria-label={t("总结用的模型")}><option value="">{t("自动")}</option>{options.map((p) => <option key={p.id} value={p.id}>{summaryOptionLabel(p)}</option>)}{draft.summary_model && !options.some((p) => p.id === draft.summary_model) && <option value={draft.summary_model}>{t("{model}（当前不可用）", { model: draft.summary_model })}</option>}</select>
        </label>
        {keyTarget && <div className="summary-key-row">
          <input type="password" autoComplete="off" aria-label={t("{provider} 的 API Key", { provider: keyTarget.provider })} placeholder={t("粘贴 {env} 的值（只存到本机 dispatch env）", { env: keyTarget.env })} value={keyValue} onChange={(e) => setKeyValue(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") void saveKey(); }} />
          <button className="btn sm" disabled={keyBusy || !keyValue.trim()} onClick={() => void saveKey()}>{keyBusy ? t("保存中…") : t("保存 Key")}</button>
          ...
          {uses.map((u) => (
            <label key={u.key} className="settings-row summary-use-row">
              ...
              <span className="summary-use-right">
                {u.last ? <span className="summary-use-last muted small">{t("最近")}{u.last.tokens > 0 ? ` ${compactTokens(u.last.tokens)} token ·` : ""} {formatSummaryTime(u.last.at)}{u.last.model ? ` · ${u.last.model}` : ""}</span> : null}
      ...
      <section className="settings-card">
        <h3>{t("讨论")}</h3>
        <p className="muted small" style={{ margin: "0 0 8px" }}>{t("「讨论一个念头」里每个成员的人设和群里的规矩，进它们的系统提示；每轮只再给新消息。空着就用默认。")}</p>
        <label className="settings-row">
          <div><b>{t("群里的规矩")}</b><p>{t("发言多长、什么时候闲聊、什么时候只回 SKIP（不显示）。")}</p></div>
        ...
        {([["discuss_persona_claude", "Claude 的人设"], ["discuss_persona_codex", "Codex 的人设"], ["discuss_persona_pi", "pi 的人设"]] as const).map(([k, label]) => (
          <label key={k} className="settings-row">
            <div><b>{t(label)}</b><p>{t("一句话：关注什么、怎么表达、习惯质疑什么。")}</p></div>
      ...
      <section className="settings-card">
        <h3>{t("工作台")}</h3>
        <label className="settings-row">
          <div><b>{t("已完成任务多少天后自动归档")}</b><p>{t("完成超过这些天的任务自动打上归档标记，从已完成列和计数里移开，「已归档」里能找到，随时可取消归档。填 0 表示不自动归档（看板上仍有「归档 30 天前完成的」按钮）。")}</p></div>
          <span className="settings-num"><input type="number" min={0} max={3650} value={draft.task_archive_days} onChange={(e) => num("task_archive_days", e.target.value, 3650)} /> {t("天")}</span>
        </label>
        <label className="settings-row">
          <div><b>{t("默认展开前几个项目")}</b><p>{t("其余项目折叠成一行摘要，点一下展开。有等你回复或等待确认的项目总是展开。")}</p></div>
          <span className="settings-num"><input type="number" min={0} max={50} value={draft.home_expanded} onChange={(e) => num("home_expanded", e.target.value, 50)} /> {t("个")}</span>
        </label>
      </section>
      <section className="settings-card">
        <h3>{t("这台电脑")}</h3>
        ...
        {onScreen && <div className="settings-row">
          <div><b>{t("屏幕访问")}</b><p>{screenReady ? t("手机连上 Tailscale 后，用浏览器打开这个链接就能看到并操作这台电脑的屏幕（noVNC），登录用这台 Mac 的用户名和密码。") : t("点「配置」自动装好 noVNC 与常驻服务、开通 Tailscale HTTPS；屏幕共享这个开关只能你自己在系统设置里打开。")}</p>
            ...
            {screenResult?.manual?.length ? <p className="muted small">{t("还差一步：{what}", { what: screenResult.manual.map((m) => `${m.title}（${m.detail}）`).join("；") })}</p> : null}
          </div>
          <span className="setup-row">
            {onScreenSetup && <button className="btn sm" disabled={screenBusy} onClick={async () => { setScreenBusy(true); try { setScreenResult(await onScreenSetup()); } finally { setScreenBusy(false); } }}>{screenBusy ? t("配置中…（要下载 noVNC）") : screenReady ? t("重新配置") : t("配置")}</button>}
            <button className="btn sm" disabled={!screenReady} onClick={onScreen}>{t("复制屏幕链接")}</button>
          </span>
        </div>}
        {onCheckUpdate && <div className="settings-row">
          <div><b>{t("版本与更新")}</b><p>{t("当前 v{v}", { v: update?.current ?? "…" })}{update?.latest ? t("，最新 v{v}", { v: update.latest }) : ""}{update?.newer ? t("，有新版本") : update?.latest ? t("，已是最新") : ""}{t("。")}{update?.error ? update.error : isTauri ? t("从 GitHub Release 下载并替换应用，完成后自动重启。") : t("网页版只能查看版本；更新在 Mac 上的 Dispatch.app 里做。")}</p></div>
          <span className="setup-row">{update?.newer && !update.error && isTauri ? <button className="btn sm primary" disabled={applying} onClick={async () => { setApplying(true); try { await onApplyUpdate?.(); } finally { setApplying(false); } }}>{applying ? t("更新中…") : t("更新到 v{v}", { v: update.latest })}</button> : null}<button className="btn sm" disabled={checking} onClick={async () => { setChecking(true); try { await onCheckUpdate(); } finally { setChecking(false); } }}>{checking ? t("检查中…") : t("检查更新")}</button></span>
        </div>}
        {onSetup && <div className="settings-row">
          <div><b>{t("首次设置")}</b><p>{t("装依赖、建或接入任务板、选 Agent、同步规则与技能。跳过过的可以从这里再打开，每一步都能重跑。")}</p></div>
          <button className="btn sm" onClick={onSetup}>{t("打开首次设置")}</button>
        </div>}
        {hosts.length > 0 && <div className="settings-row host-manage">
          <div><b>{t("机器")}</b><p>{t("来自 ~/tasks/.dispatch/hosts.json；侧栏可按机器筛选。要加一台：在那台电脑上装 Dispatch，走首次设置时选「接入」并填这台的地址；或者在这台上「接入另一台」。改名会同步推给已知的机器；删除后本机不再尝试连接它。")}</p>{onSetup && <button className="btn sm" onClick={onSetup}>{t("接入另一台电脑…")}</button>}</div>
                ...
                <div key={key} className="host-row">
                  ...
                  {renamingId === key ? (
                    <>
                      ...
                      <button className="btn ghost sm" disabled={rowBusy} onClick={() => void confirmRename(h)}>{t("保存")}</button>
                      <button className="btn ghost sm" disabled={rowBusy} onClick={() => setRenamingId(null)}>{t("取消")}</button>
                    </>
                  ) : (
                    <>
                      <span className="host-name" title={h.ip}>{h.name}{h.local ? t("（本机）") : ""}</span>
                      {onRenameHost && <button className="btn ghost sm" disabled={rowBusy} onClick={() => startRename(h)}>{rowBusy ? "…" : t("改名")}</button>}
                      {onRedetectHost && !h.local && <button className="btn ghost sm" disabled={rowBusy} onClick={() => void redetectHost(h)}>{t("重新检测")}</button>}
                      {onDeleteHost && !h.local && <button className="btn ghost sm danger" disabled={rowBusy} onClick={() => void deleteHost(h)}>{pendingDeleteId === key ? t("再点一次确认删除") : t("删除")}</button>}
                    </>
                  )}
                </div>
              );
      ...
      <div className="settings-actions">
        <button className="btn primary" disabled={!dirty || busy} onClick={() => void save()}>{busy ? t("保存中…") : t("保存")}</button>
        {dirty && <button className="btn" disabled={busy} onClick={() => setDraft(settings)}>{t("还原")}</button>}
        <span className="muted small">{t("存在共享任务板上，两台 Mac 一致；命令行 dispatch settings 读写同一份。")}</span>
      </div>
    </div>
  );
}
