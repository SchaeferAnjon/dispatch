...
export const KINDS: [string, string][] = [["claude", "Claude Code"], ["codex", "Codex"], ["gemini", "Gemini CLI"], ["opencode", "OpenCode"]];
const KIND_ACTOR: Record<string, string> = { claude: "claude-code", codex: "codex" };
    ...
    <div className="overlay" onMouseDown={(e) => e.target === e.currentTarget && onClose()}>
      <div className="dialog delegate" role="dialog" aria-label="派活">
        ...
        <div className="new-session-selects">
          ...
          <label>Agent<select value={kind} onChange={(e) => setKind(e.target.value)}>{KINDS.map(([k, l]) => <option key={k} value={k}>{l}</option>)}</select></label>
        </div>
        <label>任务<select value={taskId} onChange={(e) => setTaskId(e.target.value)}><option value="">不挂任务，只发一句话</option>{candidates.map((i) => <option key={i.id} value={i.id}>{i.id} · {i.title}{i.assignee ? ` · 现在 ${actorOf(i.assignee, me)?.name ?? i.assignee}` : ""}</option>)}</select></label>
