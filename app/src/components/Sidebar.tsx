    ...
    <aside className="side">
      ...
      {hosts.length > 1 && setHostFilter && (
        <div className="hostsw side-hosts" title="看哪台机器：任务、会话、额度、统计都只看它；「全部」合并两台" onClick={(e) => e.stopPropagation()}>
          ...
          {hosts.map((h) => <button key={h.id} className={hostFilter === h.name ? "on" : ""} onClick={() => setHostFilter(h.name)} title={`${h.name} · ${h.online ? (h.local ? "这台电脑" : "在线") : "离线"}`}><span className={`dot${h.online ? " on" : ""}`} /><span className="host-short">{h.name}</span></button>)}
        </div>
      )}
