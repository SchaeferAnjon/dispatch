...
export function DiscussStage({ d, parts, leader, me, topic }: { d: Discussion; parts: Participant[]; leader: number; me: string; topic: string }) {
  const t = useT();
  ...
  const pos = (i: number) => { const a = Math.PI / 2 + (2 * Math.PI * ((i + 1) % n)) / n; return { left: `${50 + 38 * Math.cos(a)}%`, top: `${56 + 27 * Math.sin(a)}%` }; };
  ...
  return <div className={`disc-stage${d.running ? " live" : ""}`} aria-label={t("讨论现场")}>
    ...
    {seats.map((s, i) => {
      ...
      const edge = x < 30 ? " edge-l" : x > 70 ? " edge-r" : "";
      return <div key={s.key} className={`seat s-${s.state}${s.you ? " you" : ""}${edge}`} style={pos(i)}>
