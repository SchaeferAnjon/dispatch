              ...
              {byAgent.map(([agent, list]) => {
                ...
                return (
                  <section key={agent} className="fold-agent">
                    ...
                    {list.map((s) => (
                      <div key={s.session_id} className="conv opens" onClick={() => onOpenSession(s.session_id)} role="button" tabIndex={0}>
                        <div className="l1">
                        ...
                        <div className="l3 muted small"><span className="mention">{s.last_at ? `最近 ${durSince(s.last_at)}前` : ""}{Object.keys(s.tasks).length ? ` · 提到 ${Object.keys(s.tasks).slice(0, 6).join(", ")}${Object.keys(s.tasks).length > 6 ? ` 等 ${Object.keys(s.tasks).length} 个` : ""}` : ""}</span></div>
                      </div>
                    ))}
                  </section>
                );
