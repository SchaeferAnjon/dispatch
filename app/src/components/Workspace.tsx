...
export function ConversationRows({ rows, me, onOpen }: { rows: Activity[]; me: string; onOpen: (id: string) => void }) {
  return <div className="conversation-rows">{rows.map(a => <button className={`conversation-row${a.unread ? ' unread' : ''}`} key={activityKey(a)} onClick={() => onOpen(a.session_id)}>
    ...
    </div><span className="conversation-arrow" aria-hidden>›</span>
  </button>)}</div>;
}

export function Workspace({ rows, loaded, error, unavailable, me, issues, onOpen, onTask, onInbox, onSessions, onPhone }: { rows: Activity[]; loaded: boolean; error: boolean; unavailable: string[]; me: string; issues: Issue[]; onOpen: (id: string) => void; onTask: (id: string) => void; onInbox: () => void; onSessions: () => void; onPhone?: () => void }) {
  ...
  return <div className="workspace">
    <header className="workspace-heading"><div><h2>接着上次的工作</h2><p>{running.length ? `${running.length} 个会话正在进行` : '查看回复、进展和文件改动'}</p></div><span className={`live-indicator${error ? ' interrupted' : ''}`}>{error ? '更新中断 · 正在重连' : loaded ? '每 3 秒同步活动' : '正在连接会话…'}</span>{onPhone && <button className="btn sm" onClick={onPhone}>手机访问</button>}</header>
