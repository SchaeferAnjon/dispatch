...
export function ConversationRows({ rows, me, onOpen, onRead, taskContent }: { taskContent?: (a: Activity) => ReactNode; rows: Activity[]; onRead: (a: Activity) => Promise<void>; me: string; onOpen: (id: string) => void }) {
  ...
  return <div className="conversation-rows">{rows.map(a => <div className={`conversation-row${a.unread ? ' unread' : ''}`} key={activityKey(a)} data-session={activityKey(a)}><button className="conversation-main" onClick={() => onOpen(a.session_id)} aria-label={`查看并回复：${a.title}`}>
