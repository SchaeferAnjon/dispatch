...
export function ConversationRows({ rows, me, onOpen, onRead, taskContent, compact = false }: { taskContent?: (a: Activity) => ReactNode; rows: Activity[]; onRead: (a: Activity) => Promise<void>; me: string; onOpen: (id: string) => void; compact?: boolean }) {
  ...
  return <div className="conversation-rows">{rows.map(a => <div className={`conversation-row${a.unread ? ' unread' : ''}${compact ? ' compact' : ''}`} key={activityKey(a)} data-session={activityKey(a)}><button className="conversation-main" onClick={() => onOpen(a.session_id)} aria-label={`查看并回复：${a.title}`}>
    ...
    <div className="conversation-summary">
      ...
      {!compact && <div className="conversation-preview conversation-overview" title="你在这段会话里提过的要求：最初一条，以及后续追加的"><span className="conversation-caption">你说过的</span>{a.overview || '暂无足够的对话内容可整理'}</div>}
      <div className="conversation-preview"><span className="conversation-caption">{a.reply_preview ? '最新回复' : '最新进展'}</span>{conversationSummary(a)}</div>
