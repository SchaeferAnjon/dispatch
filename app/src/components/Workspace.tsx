...
export function ConversationRows({ rows, me, onOpen, onRead }: { rows: Activity[]; onRead: (a: Activity) => Promise<void>; me: string; onOpen: (id: string) => void }) {
  ...
  return <div className="conversation-rows">{rows.map(a => <div className={`conversation-row${a.unread ? ' unread' : ''}`} key={activityKey(a)}><button className="conversation-main" onClick={() => onOpen(a.session_id)} aria-label={`查看并回复：${a.title}`}>
    ...
    </div><span className="conversation-arrow" aria-hidden>›</span></button>{a.overview && a.overview.length > 120 && <details className="conversation-overview-detail"><summary>展开会话概览</summary><p>{a.overview}</p><small>从对话中的初始目标和后续需求摘录；最新结果见回复摘要。</small></details>}<div className="conversation-actions">{a.unread && a.reply_id && <button className="btn sm" disabled={reading.has(activityKey(a))} onClick={() => void read(a)} aria-label={`标记已读：${a.title}`}>已读</button>}<button className="btn sm view-conversation" onClick={() => onOpen(a.session_id)}>查看并回复</button><OpenSessionButton session={a} /></div>
  </div>)}</div>;
}
...
export function Workspace({ rows, loaded, error, unavailable, me, issues, onOpen, onTask, onInbox, onSessions, onPhone, onNew, onRead }: { rows: Activity[]; onRead: (a: Activity) => Promise<void>; loaded: boolean; error: boolean; unavailable: string[]; me: string; issues: Issue[]; onOpen: (id: string) => void; onTask: (id: string) => void; onInbox: () => void; onSessions: () => void; onPhone?: () => void; onNew: () => void }) {
  ...
  const filtered = useMemo(() => rows.filter(a => (filter === 'running' ? a.state === 'working' && !a.stale : filter === 'unread' ? a.unread : true) && `${a.title} ${a.project} ${a.cwd} ${a.agent} ${a.overview || ""} ${conversationSummary(a)}`.toLowerCase().includes(query.toLowerCase())), [rows, filter, query]);
