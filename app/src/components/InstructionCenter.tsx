...
function Center({api, hosts, onDone, onError, hostId = ""}: Props) {
  ...
  useEffect(() => { if (hostId) setHost(hostId); }, [hostId]); const [data,setData]=useState<Inventory|null>(null); const [selected,setSelected]=useState('');
  ...
  return <div className="instruction-center">
    ...
    <div className="instruction-detail">{doc?<>
      ...
      <div className="instruction-model" title="「优化」按钮会按这里的模型和规则口径生成审查提示词；不影响 Agent 实际读取哪份文件"><label title="给哪个模型审查这份规则">当前模型 <input value={model} placeholder="未检测到，可手动填写" onChange={e=>{setModel(e.target.value);setProposal(null);}} list="detected-models" /></label><datalist id="detected-models">{data?.models.filter(x=>x.model).map(x=><option key={x.agent} value={x.model}>{x.agent}</option>)}</datalist><label title="审查时按哪家 Agent 的规则格式理解：Codex 读 AGENTS.md，Claude 读 CLAUDE.md，通用则不区分">审查口径 <select value={profile} onChange={e=>{setProfile(e.target.value);setProposal(null);}}><option value="auto">按模型识别</option><option value="codex">Codex</option><option value="claude">Claude</option><option value="general">通用</option></select></label></div>
