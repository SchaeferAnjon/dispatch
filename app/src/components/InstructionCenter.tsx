...
export function RulesView(props: Props) {
  ...
  return <div className="instruction-page"><div className="views instruction-modes"><button className={mode === 'audit' ? 'on' : ''} onClick={() => setMode('audit')}>{t('Agent 规则')}</button><button className={mode === 'facts' ? 'on' : ''} onClick={() => setMode('facts')}>{t('常用资料')}</button><button className={mode === 'memories' ? 'on' : ''} onClick={() => setMode('memories')}>{t('Agent 记忆')}</button><button className={mode === 'profile' ? 'on' : ''} onClick={() => setMode('profile')}>{t('关于我')}</button></div>{mode === 'facts' ? <FactsView {...props} /> : mode === 'memories' ? <MemoriesView {...props} /> : mode === 'profile' ? <ProfileView {...props} /> : <Center {...props} />}</div>;
}
...
function Center({api, hosts, onDone, onError, hostId = ""}: Props) {
  ...
  const save=async()=>{
    ...
    try {const raw=await api.on(host,['rules','apply',...scopeArgs,'--json'],JSON.stringify({path:doc.path,content:expandBlock(draft,block),hashes:proposal.hashes,profile,model}));const result=JSON.parse(raw.replace(/^[^{]*/,''));if(result.error){setError(result.error);return;}setBackup(result.backup||'');setDraft(null);setProposal(null);onDone(t('已保存 {n} 个文件，已保留恢复版本', { n: result.changed }));await load();}
    catch(e){setError(String(e));}finally{setBusy(false);}
  };
  const restore=async()=>{setBusy(true);try{await api.on(host,['rules','restore','--backup',backup,'--json']);setBackup('');setDraft(null);setProposal(null);onDone(t('已恢复保存前的文档'));await load();}catch(e){onError(String(e));}finally{setBusy(false);}};
  const report=proposal||data;
  const stale=syncStatus?.targets.filter(t=>t.state!=='synced').length||0;
  const sync=async()=>{setBusy(true);try{await api.on(host,['rules','sync','--json']);onDone(t('共同规则已同步到各 Agent'));await load();}catch(e){setError(String(e));}finally{setBusy(false);}};
  ...
  useItemMenu("doc", (path) => {
    ...
    return { title: d.name, items: [
      { label: t("查看"), onClick: () => choose(d.path) },
      ...(host === "local" && d.exists ? [
        { label: t("用编辑器打开"), onClick: () => api.openPath(d.path).catch((e) => onError(String(e))) },
        { label: t("在访达中打开"), onClick: () => api.openPath(d.path.replace(/\/[^/]+$/, "")).catch((e) => onError(String(e))) },
      ] : []),
      { label: t("复制路径"), onClick: () => api.copy(d.path).then(() => onDone(t("路径已复制"))) },
      ...(d.exists ? [{ label: t("复制内容"), onClick: () => api.copy(d.content || "").then(() => onDone(t("内容已复制"))) }] : []),
    ] };
  }, [data, host, api]);
  useViewMenuExtras([
    { label: t("重新检测规则文件"), onClick: () => void load() },
    { label: t("同步共同规则到各 Agent"), onClick: () => { void sync(); } },
  ], [load, sync]);
  return <div className="instruction-center">
    <div className="instruction-top"><HostPicker locked fromSidebar={!!hostId} hosts={hosts} value={host} onChange={h=>{if(!busy&&draft===null){setProject('');setSelected('');setHost(h);}}}/><label className="rule-scope">{t('作用范围')} <select aria-label={t('规则作用范围')} value={project} disabled={busy||draft!==null} onChange={e=>{setSelected('');setProject(e.target.value);}}><option value="">{t('全局规则')}</option>{projects.map(p=><option key={p.key} value={p.key}>{p.name}</option>)}</select></label><span className="spacer"/><button className="btn sm" disabled={busy||!!blocked||draft!==null||!syncStatus?.hash} onClick={sync} title={t('选择规则 → 检查并预览 → 保存；共同规则（GLOBAL.md）的修改会同步写到各 Agent 的入口文件')}>{!syncStatus?t('正在读取同步状态…'):!syncStatus.hash?t('尚未配置共同规则'):stale?t('同步共同规则 · {n} 个待更新', { n: stale }):t('共同规则已同步')}</button><button className="btn sm" disabled={busy||draft!==null} onClick={()=>void load()}>{t('重新检测')}</button>{backup&&<button className="btn sm" disabled={busy} onClick={restore}>{t('恢复上次保存')}</button>}</div>
    {blocked||error?<div className="err">{blocked||error}</div>:null}
    <div className="instruction-grid"><aside className="instruction-docs"><h3>{project?t('项目与继承的全局规则'):t('全局规则')} <span className="muted">{data?.documents.length??'…'}</span></h3>{data?.documents.map(d=><button key={d.path} data-menu="doc" data-id={d.path} className={selected===d.path?'on':''} onClick={()=>choose(d.path)} disabled={busy||draft!==null}><b>{d.name==='GLOBAL.md'?t('所有 Agent 的共同规则'):d.name}</b><span>{d.agents.join(' · ')} · {!d.exists?t('未创建'):!d.active?t('被覆盖'):d.referenced_by.length?t('引用文档'):project&&d.path.startsWith((projects.find(p=>p.key===project)?.dir||'!')+'/')?t('项目规则'):t('全局入口')}</span><small>{short(d.path)}</small></button>)}{data?.documents.length===0&&<p>{t('未找到已安装 Agent 的指令文件。支持 Codex、Claude、pi、Gemini 等全局目录。')}</p>}</aside>
    <div className="instruction-detail">{doc?<>
      <header><div><h3>{doc.name}</h3><div className="muted mono small">{short(doc.path)} · {t('{n} 行', { n: doc.lines })} · {(doc.bytes/1024).toFixed(1)} KB</div></div><span className="spacer"/><button className="btn primary sm" disabled={busy||!doc.writable} onClick={()=>{const c=collapseBlock(doc.content||'');setBlock(c.block);setDraft(c.text);setProposal(null);}}>{t('编辑')}</button><button className="btn sm" disabled={busy||!doc.exists} onClick={()=>void check(true)} title={t('本地静态检查：重复、冲突候选、失效引用、长度。不调用模型，也不改文件')}>{t('检查')}</button></header>
      {doc.real_path!==doc.path&&<p className="small muted">{t('软链接指向 {path}，保存会保留软链接。', { path: short(doc.real_path) })}</p>}
      {doc.managed&&<p className="small muted">{t('包含自动生成的托管块。共同内容请编辑其源文件；保存源文件时会预览所有受影响副本。')}</p>}
      {draft===null?<div className="instruction-content"><Markdown src={doc.content||t('尚未创建，可点击编辑写入。')}/></div>:<textarea aria-label={t('指令文档草稿')} className="instruction-editor" spellCheck={false} value={draft} onChange={e=>{setDraft(e.target.value);setProposal(null);}}/>}
      {draft!==null&&<div className="instruction-actions"><button className="btn sm" disabled={busy} onClick={()=>{setDraft(null);setProposal(null);}}>{t('取消编辑')}</button><button className="btn sm" disabled={busy} onClick={()=>void check()}>{t('检查并预览差异')}</button><button className="btn primary sm" disabled={busy||!proposal?.changes.length} onClick={save}>{t('应用 {n} 个文件的修改', { n: proposal?.changes.length||0 })}</button></div>}
      {proposal&&<div className="instruction-proposal"><h3>{t('修改预览')}</h3>{proposal.changes.length===0?<p className="muted">{t('没有可自动合并的相邻重复项。可参考下方建议编辑，再检查差异。')}</p>:proposal.changes.map(c=><details key={c.path} open><summary>{short(c.path)}</summary><pre className="diff">{c.diff}</pre></details>)}</div>}
      <section className="instruction-findings"><h3>{t('检查建议')} <span className="muted">{findings.length}</span></h3><p className="small muted">{t('本地检查不会调用模型或上传文档。潜在语义冲突需要审阅；可复制深度审查指令交给当前 Agent。')}</p>{report?.notes.map((n,i)=><p key={i} className="small muted">{n}</p>)}{findings.map((f,i)=><article key={i}><span className={`st sm ${f.kind==='conflict'?'prog':'open'}`}>{LABEL[f.kind]?t(LABEL[f.kind]):f.kind}</span><b>{f.path!==selected?`${short(f.path)} · `:''}{t('第 {n} 行', { n: f.line })} · {f.message}</b><p>{f.suggestion}</p>{f.other&&<button className="link small" onClick={()=>choose(f.path===selected?f.other!.path:f.path)}>{f.path===selected?`${short(f.other.path)}:${f.other.line}`:t('查看 {target}', { target: `${short(f.path)}:${f.line}` })}</button>}</article>)}{!findings.length&&<p className="muted">{t('当前规则集内未检出问题；不代表已完成全部语义审查。')}</p>}<button className="btn sm" onClick={()=>api.copy(prompt()).then(()=>onDone(t('深度审查指令已复制，包含检测到的有效文档'))).catch(e=>onError(String(e)))}>{t('复制深度审查指令')}</button></section>
      <details className="instruction-refs"><summary>{t('引用关系与检查依据')}</summary>{doc.references.map(p=><button className="link" key={p} onClick={()=>choose(p)}>{short(p)}</button>)}{Object.entries(data?.sources||{}).map(([k,v])=><p key={k}><a href={v} target="_blank" rel="noreferrer">{t('{name} 官方文档', { name: k })}</a></p>)}</details>
    </>:<div className="empty">{busy?t('正在检测全局文档…'):t('选择一个文档')}</div>}</div></div>
  </div>;
}
