...
export function RulesView(props: Props) {
  ...
  return <div className="instruction-page"><div className="views instruction-modes"><button className={mode === 'audit' ? 'on' : ''} onClick={() => setMode('audit')}>Agent 规则</button><button className={mode === 'facts' ? 'on' : ''} onClick={() => setMode('facts')}>常用资料</button><button className={mode === 'memories' ? 'on' : ''} onClick={() => setMode('memories')}>Agent 记忆</button><button className={mode === 'profile' ? 'on' : ''} onClick={() => setMode('profile')}>关于我</button></div>{mode === 'facts' ? <FactsView {...props} /> : mode === 'memories' ? <MemoriesView {...props} /> : mode === 'profile' ? <ProfileView {...props} /> : <Center {...props} />}</div>;
}
