...
export function FactsView({ api, hosts, onDone, onError, hostId = "" }: Props) {
  ...
  const save = async () => {
    ...
    setBusy(true);
    if (!meta) return;
    try { await api.on(host, ["facts", "write", "--path", meta.path], draft); setDraft(null); onDone(meta.key === "通用" ? t("已保存；新开的会话会带上") : t("已保存到 {path}（记得 commit）", { path: short(meta.path) })); await loadDocs(); await load(); }
    catch (e) { onError(String(e)); } finally { setBusy(false); }
  };

  return <div className="instruction-center">
    <div className="instruction-top"><HostPicker locked fromSidebar={!!hostId} hosts={hosts} value={host} onChange={(h) => { if (!busy && draft === null) setHost(h); }} /><span className="spacer" /><button className="btn sm" disabled={busy || draft !== null} onClick={() => { if (panel === "keys") setKeysRevision(n => n + 1); else { void loadDocs(); void load(); } }}>{t("重新读取")}</button></div>
    {blocked || error ? <div className="err">{blocked || error}</div> : null}
    <div className="facts-mobile-tabs views" role="tablist" aria-label={t("常用资料分类")}><button role="tab" disabled={draft!==null} aria-selected={panel === "keys"} onClick={() => setPanel("keys")}>{t("密钥/API")}</button><button role="tab" disabled={draft!==null} aria-selected={panel === "docs"} onClick={() => setPanel("docs")}>{t("服务器/数据库")}</button><button role="tab" disabled={draft!==null} aria-selected={panel === "vaults"} onClick={() => setPanel("vaults")}>Obsidian</button></div>
    <div className={`instruction-grid facts-grid mobile-${panel}`}>
      <aside className="instruction-docs facts-sidebar">
        <button className={`facts-keys-link${panel === "keys" ? " on" : ""}`} disabled={draft!==null} onClick={() => setPanel("keys")}><b>{t("密钥与 API")}</b><span>{t("Token、密码、API Key 与用途")}</span></button>
        <button className={panel === 'docs' ? 'on' : ''} disabled={draft!==null} onClick={() => {setSelected('通用');setPanel('docs');}}><b>{t("服务器与数据库")}</b><span>{t("机器、云服务、连接信息与个人资料")}</span></button>
        <button className={panel === 'vaults' ? 'on' : ''} disabled={draft!==null} onClick={() => setPanel('vaults')}><b>{t("Obsidian 资料库")}</b><span>{t("你的笔记和个人知识库")}</span></button>
      </aside>
      <div className="instruction-detail">
        {panel === "keys" ? <section className="facts-key-content" aria-label={t("密钥管理")}>
          <header><h3>{t("密钥与 API")}</h3><p className="small muted">{hosts.find(h => h.id === host)?.name || t("当前机器")}{t(" · 按名称或用途搜索，查看和修改都在这里。")}</p></header>
          <EnvKeys key={keysRevision} api={api} host={host} blocked={blocked} onDone={onDone} onError={onError} compact />
        </section> : panel === 'vaults' ? <section className="vaults-content"><h3>{t("Obsidian 资料库")}</h3><p className="small muted">{t("笔记在 Obsidian 中编辑。这里列出所选电脑的资料库；打开时使用当前设备的 Obsidian。")}</p>{vaults.length ? vaults.map(v=><article className="vault-card" key={v.id}><h4>{v.name}</h4><p className="mono small muted">{short(v.path)}</p><div><a className="btn sm" href={`obsidian://open?vault=${encodeURIComponent(v.name)}`} onClick={e=>{if(isTauri){e.preventDefault();void api.openPath(`obsidian://open?vault=${encodeURIComponent(v.name)}`).catch(err=>onError(String(err)));}}}>{t("在 Obsidian 打开")}</a><button className="btn sm" onClick={()=>api.copy(v.path).then(()=>onDone(t('资料库路径已复制'))).catch(e=>onError(String(e)))}>{t("复制路径")}</button></div>{!v.exists&&<p className="small muted">{t("当前机器暂时无法访问这个目录")}</p>}</article>):<p className="empty">{t("尚未检测到资料库。先在这台电脑的 Obsidian 中打开你的资料库，再点「重新读取」。")}</p>}</section> : <>
        <header><div><h3>{t("服务器与数据库")}</h3><div className="muted mono small">{doc ? short(doc.path) : "…"}{doc && !doc.exists ? ` · ${t("尚未创建")}` : ""}</div></div><span className="spacer" />
          {draft === null
            ? <button className="btn primary sm" disabled={busy || !doc || !meta} onClick={() => setDraft(doc?.content || t(GENERAL_TEMPLATE))}>{t("编辑")}</button>
        ...
        <p className="small muted">{t("记录服务器、数据库、账号用途和常用资料；密码与 Token 在左侧「密钥与 API」管理。Agent 的行为要求和项目文档在「Agent 规则」。")}</p>
        {draft === null
          ? <div className="instruction-content facts-content">{doc?.content ? <Markdown src={doc.content} /> : <span className="muted">{t("尚未创建，点「编辑」用模板开始。")}</span>}</div>
          : <textarea aria-label={t("常用信息草稿")} className="instruction-editor" spellCheck={false} value={draft} onChange={(e) => setDraft(e.target.value)} />}
        </>}
      </div>
    </div>
  </div>;
