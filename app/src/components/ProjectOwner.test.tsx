...
it('shows Apple and offers migration to 书房的 Mac even when history mostly lives on 书房的 Mac', () => {
  vi.stubGlobal('location', { search: '' });
  const noop = () => {};
  const rows = [{ project: 'kanban', host: 'hub', cwd: '/Users/alice/Projects/kanban', last_at: 1, session_id: 'old', agent: 'codex', tasks: [] }] as unknown as Activity[];
  const html = renderToStaticMarkup(<ProjectHub
    ...
    hosts={[{ id: 'local', name: 'Apple', local: true, online: true }, { id: 'hub', name: '书房的 Mac', online: true }] as Host[]}
  ...
  expect(html).toContain('迁移到 书房的 Mac');
