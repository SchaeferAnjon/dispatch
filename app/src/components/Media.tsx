...
export interface MediaAccess { read: (ref: string) => Promise<AttachmentData>; open: (ref: string, group?: string[]) => void; thumb: (ref: string) => string | undefined }
type Thumbs = Record<string, { name: string; mime: string; size: number; data: string }>;
// Exported so a document reader can supply the same interface for images next to a .md file.
export const MediaContext = createContext<MediaAccess | null>(null);
