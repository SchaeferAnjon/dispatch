...
let issues: Issue[] = [
  ...
  // 讨论：三个 Agent 在讨论一个念头（讨论页的示例；`discuss-live` 在 on() 里给出正在进行的一轮）
  { id: "task-ds01", title: L("【讨论】sprout 要不要做「共享账本」：两个人一起记账", "[Discussion] Should sprout get a shared ledger: two people keeping one book", "[Diskussion] Soll sprout ein gemeinsames Kassenbuch bekommen: zwei Personen, ein Buch"), description: L("室友或伴侣想一起记账。要不要做、先做到什么程度？", "Flatmates or partners want to keep one book. Should we build it, and how far in the first version?", "Mitbewohner oder Paare wollen gemeinsam Buch führen. Sollen wir das bauen, und wie weit in der ersten Version?") + "\n\n参加：claude, codex, pi:glm-5.3-flash\n领队：claude", status: "in_progress", priority: 2, issue_type: "task", created_by: "user", created_at: ago(30), updated_at: ago(1), labels: ["project:sprout", "dispatch:discussion"], comment_count: 4 },
...
comments = [...comments,
  { id: "d1", issue_id: "task-ds01", author: "user", text: "【讨论】发起：user 邀请 claude, codex, pi 讨论：" + L("室友或伴侣想一起记账。要不要做、先做到什么程度？", "Flatmates or partners want to keep one book. Should we build it, and how far in the first version?", "Mitbewohner oder Paare wollen gemeinsam Buch führen. Sollen wir das bauen, und wie weit in der ersten Version?"), created_at: ago(30) },
  { id: "d2", issue_id: "task-ds01", author: "codex", text: "【讨论】codex：" + L("先别碰实时同步。iCloud 共享数据库（CKShare）能把一个账本分享给另一个 Apple ID，冲突按记录合并，我们现在的同步层改动最小。", "Leave real-time sync alone for now. An iCloud shared database (CKShare) can share one ledger with another Apple ID and merges conflicts per record; it needs the fewest changes to our sync layer.", "Echtzeit-Sync erst einmal nicht anfassen. Eine geteilte iCloud-Datenbank (CKShare) teilt ein Kassenbuch mit einer zweiten Apple-ID und führt Konflikte pro Datensatz zusammen; unsere Sync-Schicht ändert sich dabei am wenigsten."), created_at: ago(27) },
  { id: "d3", issue_id: "task-ds01", author: "pi", text: "【讨论】pi（glm-5.3-flash）：" + L("最小版本：一个账本只能两个人，谁记的显示头像，不做分摊计算。分摊留到有人要再说。", "Smallest version: a ledger has exactly two people, each entry shows who made it, no bill splitting. Splitting waits until someone asks.", "Kleinste Version: Ein Kassenbuch hat genau zwei Personen, jeder Eintrag zeigt, wer ihn gemacht hat, keine Kostenaufteilung. Die kommt erst, wenn jemand danach fragt."), created_at: ago(25) },
  { id: "d4", issue_id: "task-ds01", author: "claude-code", text: "【讨论】claude：" + L("两位的方案能合起来：CKShare + 只限两人。验收先定三条：邀请链接能加入、两端同时改同一条不丢数据、退出共享后各自保留一份。", "The two proposals fit together: CKShare, limited to two people. Three acceptance checks first: the invite link joins, editing the same entry on both sides loses nothing, and leaving the share keeps a copy for each.", "Beide Vorschläge passen zusammen: CKShare, begrenzt auf zwei Personen. Zuerst drei Abnahmekriterien: Der Einladungslink funktioniert, gleichzeitiges Bearbeiten desselben Eintrags verliert nichts, und nach dem Verlassen behält jeder eine Kopie."), created_at: ago(22) },
];
let seq = 0;
...
export function fixtureApi(): Api {
  return {
    ...
    on: async (_h, args) => {
      ...
      if (cmd === "discuss-live") return JSON.stringify(String(sub) === "task-ds01" ? { round: 2, started: now / 1000 - 40, at: Date.now() / 1000, members: {
        "codex": { kind: "codex", status: "typing", at: Date.now() / 1000, text: L("邀请用系统的分享面板发，别自己做邀请码。对方点链接，CloudKit 会处理接受流程，我们只要在 App 里响应 userDidAcceptCloudKitShare", "Send the invite through the system share sheet, not our own invite codes. The other person taps the link, CloudKit handles acceptance, and the app only has to respond to userDidAcceptCloudKitShare", "Die Einladung über das Teilen-Menü des Systems verschicken, keine eigenen Einladungscodes. Die andere Person tippt auf den Link, CloudKit übernimmt die Annahme, die App muss nur auf userDidAcceptCloudKitShare reagieren") },
