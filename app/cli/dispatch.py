...
# OpenCode and ZCode (an OpenCode-based desktop app) keep conversations in one SQLite file each, with
# the same session / message / part tables; every query below runs against either store by agent name.
# `seq` — ZCode added a `sequence` column for ordering parts; OpenCode orders by id. `{seq}` in a query
...
def _mod(name):
    """A sibling module (profile / memories / lineage / summarize …), imported on first use so
    `dispatch` stays quick to start."""
...
def zcode_query(sql, params=(), agent="zcode"):
...
def zcode_live(table):
    """ZCode / OpenCode have no hooks; while their process runs, a session updated in the last few
    minutes counts as live (working if it wrote within 90 s)."""
    now = time.time()
    out = []
    for agent, store in SQLITE_STORES.items():
        pids = [pid for pid, (_, comm) in table.items() if os.path.basename(comm) == store["proc"]]
        if not pids:
            continue
        rows = zcode_query("select id, directory, title, time_created, time_updated from session where parent_id is null and time_archived is null and time_updated > ? order by time_updated desc", ((now - 30 * 60) * 1000,), agent=agent)
        for r in rows:
            last = r["time_updated"] / 1000
            state = "working" if now - last < 90 else "idle"
            out.append({"agent": agent, "session_id": r["id"], "cwd": r["directory"], "project": os.path.basename(r["directory"].rstrip("/")), "agent_pid": pids[0], "source_kind": store["entrypoint"], "source_app": store["app"], "entrypoint": "", "started_at": r["time_created"] / 1000, "last_at": last, "state": state, "prompts": 0, "alive": True, "registered": True, "title": r["title"]})
    return out
...
def cmd_serve(a):
    ...
    if what in ("install", "uninstall", "status"):
        args = [what] + (["--lan"] if getattr(a, "lan", False) else []) + (["--no-lan"] if getattr(a, "no_lan", False) else []) + (["--json"] if getattr(a, "json", False) else [])
    if what == "login-link":
        args = ["login-link", "--ttl", str(getattr(a, "ttl", 300) or 300)] + (["--to", a.to] if getattr(a, "to", "") else [])
    if what == "host":
...
def parse_zcode_stats(e, sid, agent="zcode"):
    for m in zcode_query("select data, time_created from message where session_id=?", (sid,), agent=agent):
        try:
            d = json.loads(m["data"])
        except Exception:
            continue
    ...
    for r in zcode_query("select json_extract(data,'$.state.input.skill') sk from part where session_id=? and json_extract(data,'$.type')='tool' and json_extract(data,'$.tool')='Skill'", (sid,), agent=agent):
        if r["sk"]:
...
def refresh_index():
    ...
    for path, agent in files:
        ...
        idx[path] = e
    # ZCode / OpenCode sessions live in SQLite, not files; key them as <agent>:<id>.
    for agent, store in SQLITE_STORES.items():
      for r in zcode_query("select id, parent_id, directory, title, time_created, time_updated from session where parent_id is null", agent=agent):
        key = f"{agent}:{r['id']}"
        seen.add(key)
        ...
        if e and e.get("mtime") == mtime and e.get("stats_v") == STATS_V:
            continue
        e = {"agent": agent, "session_id": r["id"], "cwd": r["directory"], "title": r["title"], "mtime": mtime, "size": 0, "off": 0, "tasks": {}, "claims": [], "subagent": False, "entrypoint": store["entrypoint"], "branch": "", "first_ts": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime(r["time_created"] / 1000)), "last_ts": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime(mtime)), "user_msgs": 0, "assistant_msgs": 0, "tools": {}, "first_prompt": "", "stats_v": STATS_V, **stats_fields()}
        parse_zcode_stats(e, r["id"], agent)
        fp = zcode_query("select p.data from part p join message m on m.id=p.message_id where p.session_id=? and json_extract(m.data,'$.role')='user' and json_extract(p.data,'$.type')='text' order by p.time_created limit 1", (r["id"],), agent=agent)
        if fp:
            try:
                e["first_prompt"] = (json.loads(fp[0]["data"]).get("text") or "").strip()[:240]
            except Exception:
                pass
        for m in zcode_query("select json_extract(data,'$.role') role, count(*) n from message where session_id=? group by role", (r["id"],), agent=agent):
            if m["role"] == "user":
                e["user_msgs"] = m["n"]
            elif m["role"] == "assistant":
                e["assistant_msgs"] = m["n"]
        for t in zcode_query("select json_extract(data,'$.tool') tool, count(*) n from part where session_id=? and json_extract(data,'$.type')='tool' group by tool", (r["id"],), agent=agent):
            if t["tool"]:
                e["tools"][t["tool"]] = t["n"]
        blob = "\n".join(p["data"] for p in zcode_query("select data from part where session_id=? and json_extract(data,'$.type') in ('text','tool')", (r["id"],), agent=agent))
...
def read_zcode_detail(ref, limit, since=None):
    ...
    rows = zcode_query(sql + " order by p.time_created, p.{seq}", tuple(params), agent=ref.get("agent") or "zcode")
...
def _claude_oauth_token():
    ...
    global _CLAUDE_TOKEN_NOTE
    _CLAUDE_TOKEN_NOTE = ""
    try:
        import getpass
        r = subprocess.run(["security", "find-generic-password", "-s", "Claude Code-credentials", "-a", getpass.getuser(), "-w"],
                           capture_output=True, text=True, timeout=10)
        raw = r.stdout.strip()
        if r.returncode != 0:
            # macOS asks 「python3 想访问钥匙串」 the first time; a refusal used to leave the quota page
            # silently empty. Say what it was for and how to allow it.
            _CLAUDE_TOKEN_NOTE = ("没读到 Claude Code 的登录信息：钥匙串弹窗选了「拒绝」，或这台电脑还没登录过 Claude Code。Dispatch 只用它向 Anthropic 查询你的官方用量，"
                                  "只读、不保存、不外传；下次弹窗选「始终允许」就会显示 5 小时 / 每周用量")
...
def quota_claude():
    ...
    try:
        ...
        return {"agent": "claude-code", "plan": "", "windows": [], "updated_at": None, "source": "statusline", "note": _CLAUDE_TOKEN_NOTE or "还没拿到数据：Claude Code 新会话开一句话后状态栏会写入"}
...
# ---------------------------------------------------------------- profile: 关于我 → profile.py
def cmd_profile(a):
    return _mod("profile").cmd_profile(a)



# ---------------------------------------------------------------- agent memories → memories.py
def cmd_memories(a):
    return _mod("memories").cmd_memories(a)
...
ENV_SH = os.path.join(ENV_DIR, "env.sh")         # the same for zsh / bash
...
def _write_private(path, text):
    """Secrets never exist on disk with open permissions, not even for a moment: the file is
    created 0600, written next to its destination and moved into place in one step."""
...
def env_write(items):
    ...
    body = "# Dispatch 环境/密钥库：`dispatch env` 维护；Agent 用 `dispatch env get <NAME>` 取值。不要复制到其他文件。\n\n"
    for it in items:
        if it.get("project"):
            body += f"# project: {it['project']}\n"
        if it.get("note"):
            body += f"# {it['note']}\n"
        body += f"{it['name']}={env_quote(it['value'])}\n\n"
    _write_private(ENV_FILE, body)
    _write_private(ENV_FISH, "# generated by `dispatch env`; sourced from config.fish\n" + "".join(f"set -gx {it['name']} {env_quote(it['value'])}\n" for it in items))
    # zsh / bash: `source ~/.config/dispatch/env.sh` from ~/.zshrc (macOS's default shell is zsh).
    _write_private(ENV_SH, "# generated by `dispatch env`; source it from ~/.zshrc or ~/.bashrc\n" + "".join(f"export {it['name']}={sh_quote(it['value'])}\n" for it in items))
...
# ---------------------------------------------------------------- here / lineage → lineage.py
def cmd_here(a):
    return _mod("lineage").cmd_here(a)


def cmd_lineage(a):
    return _mod("lineage").cmd_lineage(a)
...
def cmd_env(a):
    ...
    if a.op == "path":
        ...
        print(f"{a.name} 已保存（{env_mask(value)}）" + (f"，属于项目 {tag}" if tag else "") + f"。Agent 用 `dispatch env get {a.name}` 取；要在自己的终端里直接当环境变量用：zsh/bash 在 ~/.zshrc 里加一行 `source ~/.config/dispatch/env.sh`，fish 加 `source ~/.config/dispatch/env.fish`。")
    elif a.op == "unset":
        if a.name not in by:
...
def cmd_prime(a):
    ...
    lines.append('关于我：关于用户本人、设备现状、将来安排的新信息 → `dispatch profile add "…"` / `dispatch profile upcoming "YYYY-MM-DD|下次回国|待办" "…"`（`dispatch profile show` 看全文）。')
    due_soon = _mod("profile").profile_upcoming_due(_mod("profile").profile_text(), 14)
    if due_soon:
        lines.append("## 关于我（14 天内到期）")
        for r in due_soon:
            left = f"已过期 {abs(r['days_left'])} 天" if r["overdue"] else f"{r['days_left']} 天后"
            lines.append(f"- {r['when']} · {r['text']}（{left}）")
...
def main():
    ...
    s = sub.add_parser("serve", help="serve the web/phone version of Dispatch over HTTP (Tailscale); `serve url` prints the link, `serve qr` prints a scannable QR, `serve host [<id>|local]` shows/sets which Mac the phone link points at"); s.add_argument("what", nargs="?", choices=["run", "url", "qr", "host", "install", "uninstall", "status", "login-link"], default="run"); s.add_argument("--to", default="", help="login-link: 登录后打开的页面，如 #/sessions/<id>"); s.add_argument("--ttl", type=int, default=300, help="login-link: 有效秒数"); s.add_argument("target", nargs="?", default=None, help="host: a hosts.json id/name, or `local` for this Mac"); s.add_argument("--svg", action="store_true", help="qr: print SVG instead of terminal blocks"); s.add_argument("--json", action="store_true"); s.add_argument("--lan", action="store_true", help="install: 没有 Tailscale 时也允许同一局域网的设备访问"); s.add_argument("--no-lan", action="store_true", help="install: 收回局域网访问"); s.set_defaults(fn=cmd_serve)
    ...
    s = sub.add_parser("zcode-plugin", help="ZCode 插件：会话开始注入 dispatch prime 并带 task-board 技能（install | status | remove）"); s.add_argument("op", nargs="?", choices=["install", "status", "remove"], default="status"); s.add_argument("--json", action="store_true"); s.set_defaults(fn=lambda a: _mod("zcode_plugin").command(a))
    s = sub.add_parser("memories", help="各 Agent 自己的长期记忆（Claude Code / Codex / ZCode）：列出、查看、归档、让模型写总结"); s.add_argument("op", nargs="?", choices=["list", "show", "archive", "summary"], default="list"); s.add_argument("query", nargs="?", default="", help="show: 文件路径"); s.add_argument("--agent", default="", help="list: claude-code | codex | zcode"); s.add_argument("--path", default="", help="show / archive: 文件路径"); s.add_argument("--project", "-P", default="", help="summary: 只看这个项目"); s.add_argument("--force", action="store_true", help="summary: 重新生成，不用缓存"); s.add_argument("--json", action="store_true"); s.set_defaults(fn=cmd_memories)
