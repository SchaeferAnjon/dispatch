...
PROFILE_FILE = os.path.join(HOME, ".agents", "rules", "PROFILE.md")
PROFILE_INVENTORY_STATE = os.path.join(DISPATCH_DIR, "profile-inventory.json")
PROFILE_SSH_CONFIG = os.path.join(HOME, ".ssh", "config")
...
def profile_inventory_targets():
    ...
    rows = [{"name": local_host_name(), "kind": "local", "ssh": ""}]
    for h in hosts():
        rows.append({"name": h.get("name") or h.get("id") or "", "kind": "mac", "ssh": h.get("ssh") or ""})
    for alias in profile_ssh_aliases(facts_text()):
...
def profile_inventory_spawn():
    ...
    try:
        subprocess.Popen(args, cwd=HOME, stdin=subprocess.DEVNULL, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, start_new_session=True)
    except Exception:
        pass
...
def profile_inventory_due(now=None):
    ...
    if last and now - last < 86400:
        return {"due": False, "reason": f"不到一天前盘点过（{ago(last)}）"}
...
def cmd_profile(a):
    ...
    if op == "show":
        ...
        out(res, a.json, lambda x: print(x["content"], end="" if x["content"].endswith("\n") else "\n"))
        return
    if op == "add":
        ...
        profile_write_content(text)
        out({"path": PROFILE_FILE, "entry": entry, "content": text}, a.json, lambda x: print(f"已记录：{x['entry']}"))
        return
    if op == "upcoming":
        ...
        profile_write_content(text)
        out({"path": PROFILE_FILE, "entry": entry, "content": text}, a.json, lambda x: print(f"已记入将来：{x['entry']}"))
        return
    if op == "done":
        ...
        profile_write_content(text)
        out({"path": PROFILE_FILE, "removed": entry, "moved": moved, "content": text}, a.json, lambda x: print(f"已移入已发生：{x['moved']}"))
        return
    if op == "inventory":
        ...
        if getattr(a, "refresh", False):
            out(profile_inventory_generate(actor), a.json, lambda x: print(x["section"]))
        else:
            out(profile_inventory_due(), a.json, lambda x: print(x["reason"]))
        return
...
def _project_name(real, names, roots):
    ...
    if real in ("/", os.path.normpath(HOME)):
        return "通用"
    return project_of_cwd(real, names, roots) or os.path.basename(real) or "通用"
...
def zcode_project_dirs():
    ...
    known = {}
    for r in zcode_query("select distinct directory from session"):
...
def memory_summary(project="", force=False):
    ...
    cache_path = os.path.join(DISPATCH_DIR, "memory-summaries.json")
    try:
...
def cmd_memories(a):
    if a.op == "archive":
        ...
        out(res, a.json, lambda r: print(f"已归档：{r['from']} → {r['to']}"))
        return
    if a.op == "summary":
        try:
        ...
        out(res, a.json, _memory_summary_text)
        return
    rows = memory_entries()
    if a.op == "show":
...
def cmd_prime(a):
    ...
    due_soon = profile_upcoming_due(profile_text(), 14)
    if due_soon:
